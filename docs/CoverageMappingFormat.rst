...
=================================
LLVM37 Code Coverage Mapping Format
=================================
...
LLVM37's code coverage mapping format is used to provide code coverage
analysis using LLVM37's and Clang's instrumenation based profiling
(Clang's ``-fprofile-instr-generate`` option).

This document is aimed at those who use LLVM37's code coverage mapping to provide
...
We start by showing how to use LLVM37 and Clang for code coverage analysis,
then we briefly desribe LLVM37's code coverage mapping format and the
way that Clang and LLVM37's code coverage tool work with this format. After
the basics are down, more advanced features of the coverage mapping format
are discussed - such as the data structures, LLVM37 IR representation and
the binary encoding.
...
* Then, run the instrumented binary. The runtime will produce a file called
  *default.profraw* containing the raw profile instrumentation data:
  ...
  ``./test``
* After that, merge the profile data using the *llvm37-profdata* tool:

  ``llvm37-profdata merge -o test.profdata default.profraw``
* Finally, run LLVM37's code coverage tool (*llvm37-cov*) to produce the code
  coverage overview for the sample source file:

  ``llvm37-cov show ./test -instr-profile=test.profdata test.c``

High Level Overview
===================

LLVM37's code coverage mapping format is designed to be a self contained
data format, that can be embedded into the LLVM37 IR and object files.
...
1. When clang compiles a source file with ``-fcoverage-mapping``, it
   ...
   This information gets embedded into the LLVM37 IR and conveniently
   ends up in the final executable file when the program is linked.

2. It is also used by *llvm37-cov* - the mapping information is extracted from an
...
LLVM37 IR Representation
======================

The coverage mapping data is stored in the LLVM37 IR using a single global
constant structure variable called *__llvm37_coverage_mapping*
with the *__llvm37_covmap* section specifier.

For example, let’s consider a C file and how it gets compiled to LLVM37:
...
.. code-block:: llvm37
  ...
  @__llvm37_coverage_mapping = internal constant { i32, i32, i32, i32, [2 x { i8*, i32, i32 }], [40 x i8] }
  { i32 2,  ; The number of function records
    ...
    [2 x { i8*, i32, i32 }] [ ; Function records
     { i8*, i32, i32 } { i8* getelementptr inbounds ([3 x i8]* @__llvm37_profile_name_foo, i32 0, i32 0), ; Function's name
       i32 3, ; Function's name length
       i32 9  ; Function's encoded coverage mapping data string length
     },
     { i8*, i32, i32 } { i8* getelementptr inbounds ([3 x i8]* @__llvm37_profile_name_bar, i32 0, i32 0), ; Function's name
       i32 3, ; Function's name length
       i32 9  ; Function's encoded coverage mapping data string length
     }],
   [40 x i8] c"..." ; Encoded data (dissected later)
  }, section "__llvm37_covmap", align 8
...
* The length of the substring that contains the encoded translation unit
  filenames is the value of the second field in the *__llvm37_coverage_mapping*
...
* The length of the substring that contains the encoded coverage mapping data
  ...
  fifth field of the *__llvm37_coverage_mapping* structure, which is the 9.

...
Second, leveraging the LLVM37 ecosystem. Microsoft will publicly document DXIL to attract domain language implementers and spur innovation. Using LLVM37-based IR offers reduced entry costs for small teams, simply because small teams are likely to use LLVM37 and Clang as their main compilation framework. We will provide DXIL verifier to check consistency of generated DXIL.
...
The following principles are used to ease reuse with LLVM37 components and aid extensibility.

* DXIL uses a subset of LLVM37 IR constructs that makes sense for HLSL.
* No modifications to the core LLVM37 IR; i.e., no new instructions or fundamental types.
* Additional information is conveyed via metadata, LLVM37 intrinsics or external functions.
* Name prefixes: 'llvm37.dx.', 'llvm37.dxil.', 'dx.', and 'dxil.' are reserved.

LLVM37 IR has three equivalent forms: human-readable, binary (bitcode), and in-memory. DXIL is a binary format and is based on a subset of LLVM37 IR bitcode format. The document uses only human-readable form to describe DXIL.

Versioning
==========

There are three versioning mechanisms in DXIL shaders: shader model, DXIL version, and LLVM37 bitcode version.

At a high-level, the shader model describes the target execution model and environment; DXIL provides a mechanism to express programs (including rules around expressing data types and operations); and LLVM37 bitcode provides a way to encode a DXIL program.
...
DXIL version must be declared exactly once per LLVM37 module (translation unit) and is valid for the entire module.
...
LLVM37 Bitcode version
--------------------

The current version of DXIL is based on LLVM37 bitcode v3.7. This encoding is necessarily implied by something outside the DXIL module.
...
Intrinsics typically refer to operations missing in the core LLVM37 IR. DXIL represents HLSL built-in functions (also called intrinsics) not as LLVM37 intrinsics, but rather as external function calls.
...
Indexable thread-local and groupshared variables are represented as variables and accessed via LLVM37 C-like pointers.
...
The type of DXIL pointer is differentiated by LLVM37 addrspace construct. The HLSL compiler will make the best effort to infer the exact pointer addrspace such that a driver compiler can issue the most efficient instruction.
...
Indexable thread-local accesses are done via LLVM37 pointer and have C-like OOB semantics.
Groupshared accesses are done via LLVM37 pointer too. The origin of a groupshared pointer must be a single TGSM allocation.
...
OOB pointer accesses in SM6.0 and later have undefined (C-like) behavior. LLVM37 memory optimization passes can be used to optimize such accesses. Where out-of-bound behavior is desired, intrinsic functions are used to access memory.
...
There is no limit on the number of virtual values in DXIL. The IR is guaranteed to be in an SSA form. For optimized shaders, the optimizer will run -mem2reg LLVM37 pass as well as perform other memory to register promotions if profitable.

Control-flow restrictions
-------------------------

The DXIL control-flow graph must be reducible, as checked by T1-T2 test. DXIL does not preserve structured control flow of DXBC. Preserving structured control-flow property would impose significant burden on third-party tools optimizing to DXIL via LLVM37, reducing appeal of DXIL.

DXIL allows fall-through for switch label blocks. This is a difference from DXBC, in which the fall-through is prohibited.

DXIL will not support the DXBC label and call instructions; LLVM37 functions can be used instead (see below). The primary uses for these are (1) HLSL interfaces, which are not supported, and (2) outlining of case-bodies in a switch statement annotated with [call], which is not a scenario of interest.
...
The functions are regular LLVM37 functions. Parameters can be passed by-value or by-reference. The functions are to facilitate separate compilation for big, complex shaders. However, driver compilers are free to inline functions as they see fit.

Identifiers
-----------

DXIL identifiers must conform to LLVM37 IR identifier rules.
...
LoadPatchConstant function in PC stage is generated only by DXBC-to-DXIL converter, to access DXBC vpc registers. HLSL compiler produces IR that references LLVM37 IR values directly.

Type System
===========

Most of LLVM37 type system constructs are legal in DXIL.
...
The HLSL compiler optimizer treats half, i16 and i8 data as data types natively supported by the hardware; i.e., saturation, range clipping, INF/NaN are done according to the IEEE standard. Such semantics allow the optimizer to reuse LLVM37 optimization passes.
...
Explicit conversions between types are supported via LLVM37 instructions.

Precise qualifier
-----------------

By default, all floating-point HLSL operations are considered 'fast' or non-precise. HLSL and driver compilers are allowed to refactor such operations. Non-precise LLVM37 instructions: fadd, fsub, fmul, fdiv, frem, fcmp are marked with 'fast' math flags.
...
Precise behavior is represented in LLVM37 instructions: fadd, fsub, fmul, fdiv, frem, fcmp by not having 'fast' math flags set. Each relevant call instruction that contributes to computation of a precise value is annotated with dx.precise metadata that indicates that it is illegal for the driver compiler to perform IEEE-unsafe optimizations.
...
The type/field annotation metadata hierarchy recursively mimics LLVM37 type hierarchy.
...
All global resources referenced by entry points of an LLVM37 module are described via named metadata dx.resources, which consists of four metadata lists of resource records::
...
The method always returns five scalar values that are aggregated in dx.types.ResRet.f32 type and extracted into scalars via LLVM37's extractelement right after the call. The first four elements are sample values and the last field is the status of operation for tiled resources. Some return values may be unused, which is easily determined from the SSA form. The driver compiler is free to specialize the sample instruction to the most efficient form depending on which return values are used in computation.
...
LLVM37's undef value is used for unused input parameters. For example, coordinates c2 and c3 in an dx.op.sample.f32 call for Texture2D are undef, as only two coordinates c0 and c1 are required.
...
* Type of resource determines which inputs must be defined. Unused inputs are passed typed LLVM37 'undef' values. This is checked by the DXIL validator.
* Offset input parameters are i8 constants in [-8,+7] range; default offset is 0.

Resource operation return types
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Many resource operations return several scalar values as well as status for tiled resource access. The return values are grouped into a helper structure type, as this is LLVM37's way to return several values from the operation. After an operation, helper types are immediately decomposed into scalars, which are used in further computation.
...
Groupshared memory atomic operations are done via LLVM37 atomic instructions atomicrmw and cmpxchg. The instructions accept only i32 addrspace(ASgs) * pointers, where ASgs is the addrspace number of groupshared variables. Atomicrmw instruction does not support 'sub' and 'nand' operations. These constraints may be revisited in the future. OOB behavior is undefined.
...
-------------------------
There is no immediate constant buffer in DXIL. Instead, indexable constants are represented via LLVM37 global initialized constants in address space ASicb.
...
Each resource memory type is expressed by a unique LLVM37 address space. The following table shows memory types and their address spaces:
...
Operations
==========
DXIL operations are represented in two ways: using LLVM37 instructions and using LLVM37 external functions. The reference list of operations as well as their overloads can be found in the attached Excel spreadsheet "DXIL Operations".

Operations via instructions
---------------------------

DXIL uses a subset of core LLVM37 IR instructions that make sense for HLSL, where the meaning of the LLVM37 IR operation matches the meaning of the HLSL operation.

The following LLVM37 instructions are valid in a DXIL program, with the specified operand types where applicable. The legend for overload types (v)oid, (h)alf, (f)loat, (d)ouble, (1)-bit, (8)-bit, (w)ord, (i)nt, (l)ong.
...
---------------------------------
Operations missing in core LLVM37 IR, such as abs, fma, discard, etc., are represented by external functions, whose name is prefixed with dx.op.
...
The HLSL language is versioned independently of DXIL, and currently follows an 'HLSL <year>' naming scheme. HLSL 2015 is the dialect supported by the d3dcompiler_47 library; a limited form of support is provided in the open source HLSL on LLVM37 project. HLSL 2016 is the version supported by the current HLSL on LLVM37 project, which removes some features (primarily effect framework syntax, backquote operator) and adds new ones (wave intrinsics and basic i64 support).
...
The DXIL program is found in a part with the following body::
  ...
  struct DxilProgramHeader {
    ...
    uint32_t BitcodeOffset;   // Offset to LLVM37 bitcode (from DxilMagic).

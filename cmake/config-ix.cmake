...
check_symbol_exists(__GLIBC__ stdio.h LLVM37_USING_GLIBC)
if( LLVM37_USING_GLIBC )
...
function(llvm37_find_program name)
  string(TOUPPER ${name} NAME)
  string(REGEX REPLACE "\\." "_" NAME ${NAME})

  find_program(LLVM37_PATH_${NAME} NAMES ${ARGV})
  mark_as_advanced(LLVM37_PATH_${NAME})
  if(LLVM37_PATH_${NAME})
    set(HAVE_${NAME} 1 CACHE INTERNAL "Is ${name} available ?")
    mark_as_advanced(HAVE_${NAME})
  else(LLVM37_PATH_${NAME})
    set(HAVE_${NAME} "" CACHE INTERNAL "Is ${name} available ?")
  endif(LLVM37_PATH_${NAME})
endfunction()

if (LLVM37_ENABLE_DOXYGEN)
  llvm37_find_program(dot)
endif ()
...
endif( LLVM37_ENABLE_FFI )

# Define LLVM37_HAS_ATOMICS if gcc or MSVC atomic builtins are supported.
include(CheckAtomic)
...
get_host_triple(LLVM37_INFERRED_HOST_TRIPLE)

set(LLVM37_HOST_TRIPLE "${LLVM37_INFERRED_HOST_TRIPLE}" CACHE STRING
    "Host on which LLVM37 binaries will run")

# Determine the native architecture.
string(TOLOWER "${LLVM37_TARGET_ARCH}" LLVM37_NATIVE_ARCH)
if( LLVM37_NATIVE_ARCH STREQUAL "host" )
  string(REGEX MATCH "^[^-]*" LLVM37_NATIVE_ARCH ${LLVM37_HOST_TRIPLE})
endif ()

if (LLVM37_NATIVE_ARCH MATCHES "i[2-6]86")
  set(LLVM37_NATIVE_ARCH X86)
elseif (LLVM37_NATIVE_ARCH STREQUAL "x86")
  set(LLVM37_NATIVE_ARCH X86)
elseif (LLVM37_NATIVE_ARCH STREQUAL "amd64")
  set(LLVM37_NATIVE_ARCH X86)
elseif (LLVM37_NATIVE_ARCH STREQUAL "x86_64")
  set(LLVM37_NATIVE_ARCH X86)
elseif (LLVM37_NATIVE_ARCH MATCHES "sparc")
  set(LLVM37_NATIVE_ARCH Sparc)
elseif (LLVM37_NATIVE_ARCH MATCHES "powerpc")
  set(LLVM37_NATIVE_ARCH PowerPC)
elseif (LLVM37_NATIVE_ARCH MATCHES "aarch64")
  set(LLVM37_NATIVE_ARCH AArch64)
elseif (LLVM37_NATIVE_ARCH MATCHES "arm64")
  set(LLVM37_NATIVE_ARCH AArch64)
elseif (LLVM37_NATIVE_ARCH MATCHES "arm")
  set(LLVM37_NATIVE_ARCH ARM)
elseif (LLVM37_NATIVE_ARCH MATCHES "mips")
  set(LLVM37_NATIVE_ARCH Mips)
elseif (LLVM37_NATIVE_ARCH MATCHES "xcore")
  set(LLVM37_NATIVE_ARCH XCore)
elseif (LLVM37_NATIVE_ARCH MATCHES "msp430")
  set(LLVM37_NATIVE_ARCH MSP430)
elseif (LLVM37_NATIVE_ARCH MATCHES "hexagon")
  set(LLVM37_NATIVE_ARCH Hexagon)
elseif (LLVM37_NATIVE_ARCH MATCHES "s390x")
  set(LLVM37_NATIVE_ARCH SystemZ)
elseif (LLVM37_NATIVE_ARCH MATCHES "wasm32")
  set(LLVM37_NATIVE_ARCH WebAssembly)
elseif (LLVM37_NATIVE_ARCH MATCHES "wasm64")
  set(LLVM37_NATIVE_ARCH WebAssembly)
else ()
  message(FATAL_ERROR "Unknown architecture ${LLVM37_NATIVE_ARCH}")
endif ()

# If build targets includes "host", then replace with native architecture.
list(FIND LLVM37_TARGETS_TO_BUILD "host" idx)
if( NOT idx LESS 0 )
  list(REMOVE_AT LLVM37_TARGETS_TO_BUILD ${idx})
  list(APPEND LLVM37_TARGETS_TO_BUILD ${LLVM37_NATIVE_ARCH})
  list(REMOVE_DUPLICATES LLVM37_TARGETS_TO_BUILD)
endif()

list(FIND LLVM37_TARGETS_TO_BUILD ${LLVM37_NATIVE_ARCH} NATIVE_ARCH_IDX)
if (NATIVE_ARCH_IDX EQUAL -1)
  message(STATUS
    "Native target ${LLVM37_NATIVE_ARCH} is not selected; lli will not JIT code")
else ()
  message(STATUS "Native target architecture is ${LLVM37_NATIVE_ARCH}")
  set(LLVM37_NATIVE_TARGET LLVM37Initialize${LLVM37_NATIVE_ARCH}Target)
  set(LLVM37_NATIVE_TARGETINFO LLVM37Initialize${LLVM37_NATIVE_ARCH}TargetInfo)
  set(LLVM37_NATIVE_TARGETMC LLVM37Initialize${LLVM37_NATIVE_ARCH}TargetMC)
  set(LLVM37_NATIVE_ASMPRINTER LLVM37Initialize${LLVM37_NATIVE_ARCH}AsmPrinter)

  # We don't have an ASM parser for all architectures yet.
  if (EXISTS ${CMAKE_SOURCE_DIR}/lib/Target/${LLVM37_NATIVE_ARCH}/AsmParser/CMakeLists.txt)
    set(LLVM37_NATIVE_ASMPARSER LLVM37Initialize${LLVM37_NATIVE_ARCH}AsmParser)
  endif ()

  # We don't have an disassembler for all architectures yet.
  if (EXISTS ${CMAKE_SOURCE_DIR}/lib/Target/${LLVM37_NATIVE_ARCH}/Disassembler/CMakeLists.txt)
    set(LLVM37_NATIVE_DISASSEMBLER LLVM37Initialize${LLVM37_NATIVE_ARCH}Disassembler)
  endif ()
endif ()
...
if (LLVM37_ENABLE_ZLIB )
  ...
  if ( NOT HAVE_ZLIB_H OR NOT HAVE_LIBZ )
    set(LLVM37_ENABLE_ZLIB 0)
  endif()
endif()

set(LLVM37_PREFIX ${CMAKE_INSTALL_PREFIX})
...
if (LLVM37_ENABLE_DOXYGEN)
  ...
  if (DOXYGEN_FOUND)
    ...
    # global aggregate doxygen target for generating llvm37 and any/all
    # subprojects doxygen documentation.
    if (LLVM37_BUILD_DOCS)
      add_custom_target(doxygen ALL)
    endif()

    option(LLVM37_DOXYGEN_EXTERNAL_SEARCH "Enable doxygen external search." OFF)
    if (LLVM37_DOXYGEN_EXTERNAL_SEARCH)
      set(LLVM37_DOXYGEN_SEARCHENGINE_URL "" CACHE STRING "URL to use for external searhc.")
      set(LLVM37_DOXYGEN_SEARCH_MAPPINGS "" CACHE STRING "Doxygen Search Mappings")
    endif()
  endif()
else()
...
if(WIN32)
  ...
  if(GO_EXECUTABLE STREQUAL "GO_EXECUTABLE-NOTFOUND")
    ...
    if(GO_CONFTEST STREQUAL "0")
      set(LLVM37_BINDINGS "${LLVM37_BINDINGS} go")
...
find_program(GOLD_EXECUTABLE NAMES ${LLVM37_DEFAULT_TARGET_TRIPLE}-ld.gold ld.gold ${LLVM37_DEFAULT_TARGET_TRIPLE}-ld ld DOC "The gold linker")
set(LLVM37_BINUTILS_INCDIR "" CACHE PATH
  ...
  if( NOT OCAML_FOUND )
    ...
    if( OCAML_VERSION VERSION_LESS "4.00.0" )
      ...
      if( HAVE_OCAML_CTYPES )
        ...
        set(LLVM37_BINDINGS "${LLVM37_BINDINGS} ocaml")
      else()
...
string(REPLACE " " ";" LLVM37_BINDINGS_LIST "${LLVM37_BINDINGS}")

//===- llvm37/Support/Debug.h - Easy way to add debug output ------*- C++ -*-===//
//
...
#ifndef LLVM37_SUPPORT_DEBUG_H
#define LLVM37_SUPPORT_DEBUG_H
...
#define DEBUG_WITH_TYPE(TYPE, X)                                        \
  do { if (::llvm37::DebugFlag && ::llvm37::isCurrentDebugType(TYPE)) { X; } \
  } while (0)

// HLSL Change
#define DEBUG_WITH_TYPE_IF(C, TYPE, X)                                        \
  do { if ((C) && ::llvm37::DebugFlag && ::llvm37::isCurrentDebugType(TYPE)) { X; __BREAK_ON_FAIL; } \
  } while (0)

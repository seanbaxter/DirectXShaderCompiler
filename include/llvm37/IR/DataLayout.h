//===--------- llvm37/DataLayout.h - Data size & alignment info ---*- C++ -*-===//
//
...
#ifndef LLVM37_IR_DATALAYOUT_H
#define LLVM37_IR_DATALAYOUT_H
...
// This needs to be outside of the namespace, to avoid conflict with llvm37-c
// decl.
...
/// generating LLVM37 IR is required to generate the right target data for the
...
public:
  ...
  char getGlobalPrefix() const {
    ...
    llvm37_unreachable("invalid mangling mode");
  }
  ...
  IntegerType *getIntPtrType(LLVM37Context &C, unsigned AddressSpace = 0) const;
  ...
  /// Width bits.
  Type *getSmallestLegalIntType(LLVM37Context &C, unsigned Width = 0) const;

  /// \brief Returns the largest legal integer type, or null if none are set.
  Type *getLargestLegalIntType(LLVM37Context &C) const {
...
inline DataLayout *unwrap(LLVM37TargetDataRef P) {
  return reinterpret_cast<DataLayout *>(P);
}

inline LLVM37TargetDataRef wrap(const DataLayout *P) {
  return reinterpret_cast<LLVM37TargetDataRef>(const_cast<DataLayout *>(P));
}
...
inline uint64_t DataLayout::getTypeSizeInBits(Type *Ty) const {
  ...
  default:
    llvm37_unreachable("DataLayout::getTypeSizeInBits(): Unsupported type");
  }
}

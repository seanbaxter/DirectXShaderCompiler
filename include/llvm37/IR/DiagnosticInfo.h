//===- llvm37/Support/DiagnosticInfo.h - Diagnostic Declaration ---*- C++ -*-===//
//
...
//
// Diagnostics reporting is still done as part of the LLVM37Context.
//===----------------------------------------------------------------------===//

#ifndef LLVM37_IR_DIAGNOSTICINFO_H
#define LLVM37_IR_DIAGNOSTICINFO_H
...
public:
  ...
  /// of the LLVM37 command line flags (-pass-remarks, -pass-remarks-missed,
  /// or -pass-remarks-analysis). Note that this only handles the LLVM37
...
DEFINE_SIMPLE_CONVERSION_FUNCTIONS(DiagnosticInfo, LLVM37DiagnosticInfoRef)
...
void emitOptimizationRemark(LLVM37Context &Ctx, const char *PassName,
...
void emitOptimizationRemarkMissed(LLVM37Context &Ctx, const char *PassName,
                                  const Function &Fn, const DebugLoc &DLoc,
                                  const Twine &Msg);
...
/// to use.
void emitOptimizationRemarkAnalysis(LLVM37Context &Ctx, const char *PassName,
                                    const Function &Fn, const DebugLoc &DLoc,
                                    const Twine &Msg);
...
void emitLoopVectorizeWarning(LLVM37Context &Ctx, const Function &Fn,
                              const DebugLoc &DLoc, const Twine &Msg);

/// Emit a warning when loop interleaving is specified but fails. \p Fn is the
/// function triggering the warning, \p DLoc is the debug location where the
/// diagnostic is generated. \p Msg is the message string to use.
void emitLoopInterleaveWarning(LLVM37Context &Ctx, const Function &Fn,
                               const DebugLoc &DLoc, const Twine &Msg);

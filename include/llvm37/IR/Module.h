//===-- llvm37/Module.h - C++ class to represent a VM module ------*- C++ -*-===//
//
...
#ifndef LLVM37_IR_MODULE_H
#define LLVM37_IR_MODULE_H
...
/// LLVM37 module. Modules are the top level container of all other LLVM37
...
/// @brief The main container class for the LLVM37 Intermediate Representation.
class Module {
...
/// @{
private:
  LLVM37Context &Context;           ///< The LLVM37Context from which types and
...
public:
  ...
  explicit Module(StringRef ModuleID, LLVM37Context& C);
...
/// @{
  ...
  /// @returns LLVM37Context - a container for LLVM37's global information
  ...
  /// addition, the random stream will be reproducible across LLVM37
...
/// @{
  ...
  /// uniqued across modules in the current LLVM37Context.
  unsigned getMDKindID(StringRef Name) const;

  /// Populate client supplied SmallVector with the name for custom metadata IDs
  /// registered in this LLVM37Context.
...
/// @{
  ...
  Constant *getOrInsertFunction(StringRef Name,
                                AttributeSet AttributeList,
                                Type *RetTy, ...) LLVM37_END_WITH_NULL;
...
/// @{
  ...
  /// Destroy ConstantArrays in LLVM37Context if they are not used.
  /// ConstantArrays constructed during linking can cause quadratic memory
  /// explosion. Releasing all unused constants can cause a 20% LTO compile-time
  /// slowdown for a large application.
  ///
  /// NOTE: Constants are currently owned by LLVM37Context. This can then only
  /// be called where all uses of the LLVM37Context are understood.
...
/// @}
  ...
  // HLSL Change start
  typedef void (*RemoveGlobalCallback)(llvm37::Module*, llvm37::GlobalObject*);
  typedef void(*ResetModuleCallback)(llvm37::Module*);
  RemoveGlobalCallback pfnRemoveGlobal = nullptr;
  void CallRemoveGlobalHook(llvm37::GlobalObject* G) {
...
DEFINE_SIMPLE_CONVERSION_FUNCTIONS(Module, LLVM37ModuleRef)

/* LLVM37ModuleProviderRef exists for historical reasons, but now just holds a
 * Module.
 */
inline Module *unwrap(LLVM37ModuleProviderRef MP) {

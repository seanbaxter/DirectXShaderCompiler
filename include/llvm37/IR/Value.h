//===-- llvm37/Value.h - Definition of the Value class ------------*- C++ -*-===//
//
...
#ifndef LLVM37_IR_VALUE_H
#define LLVM37_IR_VALUE_H
...
/// \brief LLVM37 Value Representation
///
/// This is a very important LLVM37 class. It is the base class of all values
...
/// llvm37/IR/ValueHandle.h for details.
class Value {
  Type *VTy;
  Use *UseList;
...
#include "llvm37/IR/Value.def"
  ...
  /// for any other purpose, as the values may change as LLVM37 evolves.  Also,
...
private:
  ...
  static Use *mergeUseLists(Use *L, Use *R, Compare Cmp) {
    ...
    // HLSL Change Begins. Copied from LLVM37 Version 8.0.0.
...
DEFINE_ISA_CONVERSION_FUNCTIONS(Value, LLVM37ValueRef)

/* Specialized opaque value conversions.
 */
inline Value **unwrap(LLVM37ValueRef *Vals) {
  return reinterpret_cast<Value**>(Vals);
}

template<typename T>
inline T **unwrap(LLVM37ValueRef *Vals, unsigned Length) {
#ifdef DEBUG
  for (LLVM37ValueRef *I = Vals, *E = Vals + Length; I != E; ++I)
    cast<T>(*I);
#endif
  (void)Length;
...
inline LLVM37ValueRef *wrap(const Value **Vals) {
  return reinterpret_cast<LLVM37ValueRef*>(const_cast<Value**>(Vals));
}

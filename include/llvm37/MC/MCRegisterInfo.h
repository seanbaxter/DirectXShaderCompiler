...
#ifndef LLVM37_MC_MCREGISTERINFO_H
#define LLVM37_MC_MCREGISTERINFO_H
...
public:
  ...
  /// DwarfLLVM37RegPair - Emitted by tablegen so Dwarf<->LLVM37 reg mappings can be
  /// performed with a binary search.
  struct DwarfLLVM37RegPair {
    unsigned FromReg;
    unsigned ToReg;

    bool operator<(DwarfLLVM37RegPair RHS) const { return FromReg < RHS.FromReg; }
  };
...
private:
  ...
  const DwarfLLVM37RegPair *L2DwarfRegs;        // LLVM37 to Dwarf regs mapping
  const DwarfLLVM37RegPair *EHL2DwarfRegs;      // LLVM37 to Dwarf regs mapping EH
  const DwarfLLVM37RegPair *Dwarf2LRegs;        // Dwarf to LLVM37 regs mapping
  const DwarfLLVM37RegPair *EHDwarf2LRegs;      // Dwarf to LLVM37 regs mapping EH
  DenseMap<unsigned, int> L2SEHRegs;          // LLVM37 to SEH regs mapping
...
public:
  ...
  /// \brief Used to initialize LLVM37 register to Dwarf
  /// register number mapping. Called by TableGen auto-generated routines.
  /// *DO NOT USE*.
  void mapLLVM37RegsToDwarfRegs(const DwarfLLVM37RegPair *Map, unsigned Size,
                              bool isEH) {
    if (isEH) {
      EHL2DwarfRegs = Map;
  ...
  /// \brief Used to initialize Dwarf register to LLVM37
  /// register number mapping. Called by TableGen auto-generated routines.
  /// *DO NOT USE*.
  void mapDwarfRegsToLLVM37Regs(const DwarfLLVM37RegPair *Map, unsigned Size,
                              bool isEH) {
    if (isEH) {
      EHDwarf2LRegs = Map;
  ...
  /// mapLLVM37RegToSEHReg - Used to initialize LLVM37 register to SEH register
  /// number mapping. By default the SEH register number is just the same
  /// as the LLVM37 register number.
  /// FIXME: TableGen these numbers. Currently this requires target specific
  /// initialization code.
  void mapLLVM37RegToSEHReg(unsigned LLVM37Reg, int SEHReg) {
    L2SEHRegs[LLVM37Reg] = SEHReg;
  }
  ...
  int getLLVM37RegNum(unsigned RegNum, bool isEH) const;

  /// \brief Map a target register to an equivalent SEH register
  /// number.  Returns LLVM37 register number if there is no equivalent value.

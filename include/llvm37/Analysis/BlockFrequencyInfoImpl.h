...
#ifndef LLVM37_ANALYSIS_BLOCKFREQUENCYINFOIMPL_H
#define LLVM37_ANALYSIS_BLOCKFREQUENCYINFOIMPL_H
...
template <class BT> void BlockFrequencyInfoImpl<BT>::computeMassInLoops() {
  ...
  for (auto L = Loops.rbegin(), E = Loops.rend(); L != E; ++L) {
    ...
    if (computeMassInLoop(*L))
      continue;
    llvm37_unreachable("unhandled irreducible control flow");
  }
}
...
bool BlockFrequencyInfoImpl<BT>::computeMassInLoop(LoopData &Loop) {
  ...
  if (Loop.isIrreducible()) {
    ...
    if (!propagateMassToSuccessors(&Loop, Loop.getHeader()))
      llvm37_unreachable("irreducible control flow to loop header!?");

...
#ifndef LLVM37_EXECUTIONENGINE_ORC_INDIRECTIONUTILS_H
#define LLVM37_EXECUTIONENGINE_ORC_INDIRECTIONUTILS_H
...
public:
  ...
  virtual CompileCallbackInfo getCompileCallback(LLVM37Context &Context) = 0;
...
public:
  ...
  /// @param Context LLVM37Context to use for trampoline & resolve block modules.
  /// @param ErrorHandlerAddress The address of an error handler in the target
  ///                            process to be used if a compile callback fails.
  ...
  CompileCallbackInfo getCompileCallback(LLVM37Context &Context) final {
...
private:
  ...
  void emitResolverBlock(LLVM37Context &Context) {
    ...
    auto NonResolver =
      createLambdaResolver(
          [](const std::string &Name) -> RuntimeDyld::SymbolInfo {
            llvm37_unreachable("External symbols in resolver block?");
          },
          [](const std::string &Name) -> RuntimeDyld::SymbolInfo {
            llvm37_unreachable("Dylib symbols in resolver block?");
          });
  ...
  TargetAddress getAvailableTrampolineAddr(LLVM37Context &Context) {
  ...
  void grow(LLVM37Context &Context) {
    ...
    auto NonResolver =
      createLambdaResolver(
          [](const std::string &Name) -> RuntimeDyld::SymbolInfo {
            llvm37_unreachable("External symbols in trampoline block?");
          },
          [](const std::string &Name) -> RuntimeDyld::SymbolInfo {
            llvm37_unreachable("Dylib symbols in trampoline block?");
          });
    auto H = JIT.addModuleSet(SingletonSet(std::move(M)), &MemMgr,
                              std::move(NonResolver));
    JIT.emitAndFinalize(H);
...
#endif // LLVM37_EXECUTIONENGINE_ORC_INDIRECTIONUTILS_H

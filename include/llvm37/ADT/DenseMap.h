//===- llvm37/ADT/DenseMap.h - Dense probed hash table ------------*- C++ -*-===//
//
...
#ifndef LLVM37_ADT_DENSEMAP_H
#define LLVM37_ADT_DENSEMAP_H

#include "llvm37/ADT/DenseMapInfo.h"
#include "llvm37/ADT/EpochTracker.h"
...
public:
  ...
  bool LLVM37_ATTRIBUTE_UNUSED_RESULT empty() const {
...
private:
  ...
  BucketT *InsertIntoBucketImpl(const KeyT &Key, BucketT *TheBucket) {
    ...
    if (LLVM37_UNLIKELY(NewNumEntries * 4 >= NumBuckets * 3)) {
      this->grow(NumBuckets * 2);
      LookupBucketFor(Key, TheBucket);
      NumBuckets = getNumBuckets();
    } else if (LLVM37_UNLIKELY(NumBuckets-(NewNumEntries+getNumTombstones()) <=
                             NumBuckets/8)) {
  ...
  bool LookupBucketFor(const LookupKeyT &Val,
    ...
    while (1) {
      ...
      if (LLVM37_LIKELY(KeyInfoT::isEqual(Val, ThisBucket->getFirst()))) {
      ...
      if (LLVM37_LIKELY(KeyInfoT::isEqual(ThisBucket->getFirst(), EmptyKey))) {

//===- llvm37/ADT/SmallVector.h - 'Normally small' vectors --------*- C++ -*-===//
//
...
#ifndef LLVM37_ADT_SMALLVECTOR_H
#define LLVM37_ADT_SMALLVECTOR_H
...
public:
  ...
  bool LLVM37_ATTRIBUTE_UNUSED_RESULT empty() const { return BeginX == EndX; }
};
...
private:
  ...
  typedef llvm37::AlignedCharArrayUnion<T> U;
  U FirstEl;
...
public:
  void push_back(const T &Elt) {
    if (LLVM37_UNLIKELY(this->EndX >= this->CapacityX))
      this->grow();
...
public:
  ...
  T LLVM37_ATTRIBUTE_UNUSED_RESULT pop_back_val() {
...
public:
  ...
  explicit SmallVector(const llvm37::iterator_range<RangeTy> R)
      : SmallVectorImpl<T>(N) {
...
namespace std {
  ...
  template<typename T>
  inline void
  swap(llvm37::SmallVectorImpl<T> &LHS, llvm37::SmallVectorImpl<T> &RHS) {
    LHS.swap(RHS);
  }
  ...
  template<typename T, unsigned N>
  inline void
  swap(llvm37::SmallVector<T, N> &LHS, llvm37::SmallVector<T, N> &RHS) {
    LHS.swap(RHS);
  }
}

//===-- llvm37/CodeGen/ISDOpcodes.h - CodeGen opcodes -------------*- C++ -*-===//
//
...
#ifndef LLVM37_CODEGEN_ISDOPCODES_H
#define LLVM37_CODEGEN_ISDOPCODES_H
...
namespace ISD {
  ...
  enum NodeType {
    ...
    /// FRAMEADDR, RETURNADDR - These nodes represent llvm37.frameaddress and
    /// llvm37.returnaddress on the DAG.  These nodes take one operand, the index
    /// of the frame or return address to return.  An index of zero corresponds
    /// to the current function's frame or return address, an index of one to
    /// the parent's frame or return address, and so on.
    FRAMEADDR, RETURNADDR,

    /// LOCAL_RECOVER - Represents the llvm37.localrecover intrinsic.
    /// Materializes the offset from the local object pointer of another
    /// function to a particular local object passed to llvm37.localescape. The
    ...
    /// READ_REGISTER, WRITE_REGISTER - This node represents llvm37.register on
    ...
    /// llvm37::Intrinsic namespace.  The operands to the intrinsic follow.  The
    ...
    /// the ID number of the intrinsic from the llvm37::Intrinsic namespace.  The
    ...
    /// second is the ID number of the intrinsic from the llvm37::Intrinsic
    ...
    /// These nodes are generated from llvm37.[su]add.with.overflow intrinsics.
    SADDO, UADDO,
    ...
    /// operands as an LLVM37 load/store instruction, then an offset node that
    ...
    /// make reference to a value in the LLVM37 IR.
    SRCVALUE,
    ...
    /// This corresponds to the llvm37.lifetime.* intrinsics. The first operand

//===-- llvm37/Bitcode/ReaderWriter.h - Bitcode reader/writers ----*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This header defines interfaces to read and write LLVM37 bitcode files/streams.
//
//===----------------------------------------------------------------------===//

#ifndef LLVM37_BITCODE_READERWRITER_H
#define LLVM37_BITCODE_READERWRITER_H
...
namespace llvm37 {
  ...
  /// returns "".
  std::string
  getBitcodeTargetTriple(MemoryBufferRef Buffer, LLVM37Context &Context,
                         DiagnosticHandlerFunction DiagnosticHandler = nullptr);

  /// Read the specified bitcode file, returning the module.
  ErrorOr<std::unique_ptr<Module>>
  parseBitcodeFile(MemoryBufferRef Buffer, LLVM37Context &Context,
                   DiagnosticHandlerFunction DiagnosticHandler = nullptr,
  ...
  /// for an LLVM37 IR bitcode wrapper.
  ///
  ...
  /// raw LLVM37 IR bitcode (without a wrapper).
  ///
  ...
  /// LLVM37 IR bitcode, either with or without a wrapper.
  ///
...
namespace std {
template <> struct is_error_code_enum<llvm37::BitcodeError> : std::true_type {};
}

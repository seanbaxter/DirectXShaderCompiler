...
#ifndef LLVM37_OBJECT_ELFTYPES_H
#define LLVM37_OBJECT_ELFTYPES_H
...
#define LLVM37_ELF_IMPORT_TYPES(E, W)                                            \
...
#define LLVM37_ELF_IMPORT_TYPES_ELFT(ELFT)                                       \
  LLVM37_ELF_IMPORT_TYPES(ELFT::TargetEndianness, ELFT::Is64Bits)
...
struct Elf_Shdr_Base<ELFType<TargetEndianness, false>> {
  LLVM37_ELF_IMPORT_TYPES(TargetEndianness, false)
...
struct Elf_Shdr_Base<ELFType<TargetEndianness, true>> {
  LLVM37_ELF_IMPORT_TYPES(TargetEndianness, true)
  Elf_Word sh_name;       // Section name (index into string table)
  Elf_Word sh_type;       // Section type (SHT_*)
...
template <llvm37::support::endianness TargetEndianness>
...
template <class ELFT> struct Elf_Mips_Options {
  ...
  const Elf_Mips_RegInfo<ELFT> &getRegInfo() const {
    assert(kind == llvm37::ELF::ODK_REGINFO);

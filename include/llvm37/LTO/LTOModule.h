//===-LTOModule.h - LLVM37 Link Time Optimizer ------------------------------===//
//
...
#ifndef LLVM37_LTO_LTOMODULE_H
#define LLVM37_LTO_LTOMODULE_H
...
// Forward references to llvm37 classes.
namespace llvm37 {
  class Function;
  class GlobalValue;
  class MemoryBuffer;
...
private:
  ...
  std::unique_ptr<LLVM37Context> OwnedContext;
  ...
  LTOModule(std::unique_ptr<object::IRObjectFile> Obj, TargetMachine *TM,
            std::unique_ptr<LLVM37Context> Context);

public:
  ~LTOModule();

  /// Returns 'true' if the file or memory contents is LLVM37 bitcode.
  static bool isBitcodeFile(const void *mem, size_t length);
  static bool isBitcodeFile(const char *path);

  /// Returns 'true' if the memory buffer is LLVM37 bitcode for the specified
  /// triple.
  ...
  static LTOModule *createInContext(const void *mem, size_t length,
                                    TargetOptions options, std::string &errMsg,
                                    StringRef path, LLVM37Context *Context);
...
private:
  ...
  static LTOModule *makeLTOModule(MemoryBufferRef Buffer, TargetOptions options,
                                  std::string &errMsg, LLVM37Context *Context);
};
}
#endif

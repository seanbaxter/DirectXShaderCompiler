...
//===----------------------------------------------------------------------===//
//
// Implementation for LLVM37 symbolization library.
//
//===----------------------------------------------------------------------===//
...
static bool error(std::error_code ec) {
  if (!ec)
    return false;
  errs() << "LLVM37Symbolizer: error reading file: " << ec.message() << ".\n";
  return true;
}

static DILineInfoSpecifier
getDILineInfoSpecifier(const LLVM37Symbolizer::Options &Opts) {
...
const char LLVM37Symbolizer::kBadString[] = "??";

std::string LLVM37Symbolizer::symbolizeCode(const std::string &ModuleName,
...
std::string LLVM37Symbolizer::symbolizeData(const std::string &ModuleName,
                                          uint64_t ModuleOffset) {
...
ObjectFile *LLVM37Symbolizer::lookUpDsymFile(const std::string &ExePath,
...
LLVM37Symbolizer::ObjectPair
LLVM37Symbolizer::getOrCreateObjects(const std::string &Path,
...
ObjectFile *
LLVM37Symbolizer::getObjectFileFromBinary(Binary *Bin,
                                        const std::string &ArchName) {
  if (!Bin)
    return nullptr;
...
ModuleInfo *
LLVM37Symbolizer::getOrCreateModuleInfo(const std::string &ModuleName) {
...
std::string LLVM37Symbolizer::printDILineInfo(DILineInfo LineInfo) const {
...
std::string LLVM37Symbolizer::DemangleName(const std::string &Name) {
#if !defined(_MSC_VER)

//===-- llvm-config.cpp - LLVM37 project configuration utility --------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This tool encapsulates information about an LLVM37 project configuration for
// use by other project's build environments (to determine installed path,
// available features, required libraries, etc.).
//
// Note that although this tool *may* be used by some parts of LLVM37's build
...
LLVM37.  Typically called from 'configure' scripts.  Examples:\n\
  llvm-config --cxxflags\n\
  llvm-config --ldflags\n\
  llvm-config --libs engine bcreader scalaropts\n\
\n\
Options:\n\
  --version         Print LLVM37 version.\n\
  --prefix          Print the installation prefix.\n\
  --src-root        Print the source root LLVM37 was built from.\n\
  --obj-root        Print the object root used to build LLVM37.\n\
  --bindir          Directory containing LLVM37 executables.\n\
  --includedir      Directory containing LLVM37 headers.\n\
  --libdir          Directory containing LLVM37 libraries.\n\
  --cppflags        C preprocessor flags for files that include LLVM37 headers.\n\
  --cflags          C compiler flags for files that include LLVM37 headers.\n\
  --cxxflags        C++ compiler flags for files that include LLVM37 headers.\n\
  --ldflags         Print Linker flags.\n\
  --system-libs     System Libraries needed to link against LLVM37 components.\n\
  --libs            Libraries needed to link against LLVM37 components.\n\
  --libnames        Bare library names for in-tree builds.\n\
  --libfiles        Fully qualified library filenames for makefile depends.\n\
  --components      List of all possible components.\n\
  --targets-built   List of all targets currently built.\n\
  --host-target     Target triple used to configure LLVM37.\n\
  --build-mode      Print build mode of LLVM37 tree (e.g. Debug or Release).\n\
  --assertion-mode  Print assertion mode of LLVM37 tree (ON or OFF).\n\
Typical components:\n\
  all               All LLVM37 libraries (default).\n\
...
int __cdecl main(int argc, char **argv) {
  ...
  if (PrintLibs || PrintLibNames || PrintLibFiles || PrintSystemLibs) {
    ...
    // FIXME: Each LLVM37 component may have its dependent system libs.
    if (PrintSystemLibs)

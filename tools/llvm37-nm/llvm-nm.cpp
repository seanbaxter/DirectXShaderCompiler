//===-- llvm37-nm.cpp - Symbol table dumping utility for llvm37 ---------------===//
//
...
// on the command line.  Such that "llvm37-nm -s __TEXT __text foo.o" would work.
...
cl::opt<bool> NoLLVM37Bitcode("no-llvm37-bc",
                            cl::desc("Disable LLVM37 bitcode reader"));
...
static void dumpSymbolNamesFromFile(std::string &Filename) {
  ...
  ErrorOr<std::unique_ptr<Binary>> BinaryOrErr = createBinary(
      BufferOrErr.get()->getMemBufferRef(), NoLLVM37Bitcode ? nullptr : &Context);
...
int __cdecl  main(int argc, char **argv) {
  ...
  cl::ParseCommandLineOptions(argc, argv, "llvm37 symbol table dumper\n");

  // llvm37-nm only reads binary files.

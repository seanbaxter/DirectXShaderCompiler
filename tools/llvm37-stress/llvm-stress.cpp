//===-- llvm37-stress.cpp - Generate random LL files to stress-test LLVM37 ----===//
//
...
// different components in LLVM37.
//
//===----------------------------------------------------------------------===//
...
struct BinModifier: public Modifier {
  ...
  void Act() override {
    ...
    switch (R) {
    default: llvm37_unreachable("Invalid BinOp");
...
int __cdecl main(int argc, char **argv) { // HLSL Change - __cdecl
  ...
  // Init LLVM37, call llvm37_shutdown() on exit, parse args, etc.
  PrettyStackTraceProgram X(argc, argv);
  cl::ParseCommandLineOptions(argc, argv, "llvm37 codegen stress-tester\n");

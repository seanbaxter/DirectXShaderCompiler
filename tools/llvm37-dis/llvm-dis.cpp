//===-- llvm37-dis.cpp - The low-level LLVM37 disassembler --------------------===//
//
...
//  llvm37-dis [options]      - Read LLVM37 bitcode from stdin, write asm to stdout
//  llvm37-dis [options] x.bc - Read LLVM37 bitcode from the x.bc file, write asm
//                            to the x.ll file.
//  Options:
...
int __cdecl main(int argc, char **argv) { // HLSL Change - __cdecl
  ...
  cl::ParseCommandLineOptions(argc, argv, "llvm37 .bc -> .ll disassembler\n");
  ...
  // All that llvm37-dis does is write the assembly to a file.
  if (!DontPrint)

...
void PrintStructBufferDefinition(DxilResource *buf,
  ...
  if (!RetTy->isStructTy() || HLMatrixType::isa(RetTy)) {
    llvm37::Type *Ty = buf->GetHLSLType()->getPointerElementType();
...
HRESULT Disassemble(IDxcBlob *pProgram, raw_string_ostream &Stream) {
  ...
  std::string DiagStr;
  llvm37::LLVM37Context llvm37Context;
  std::unique_ptr<llvm37::Module> pModule(dxilutil::LoadModuleFromBitcode(
    llvm37::StringRef(pIL, pILLength), llvm37Context, DiagStr));
  ...
  if (pReflectionIL && pReflectionILLength) {
    pReflectionModule = dxilutil::LoadModuleFromBitcode(
      llvm37::StringRef(pReflectionIL, pReflectionILLength), llvm37Context, DiagStr);

...
these languages. Clang builds on the LLVM37 optimizer and code generator,
allowing it to provide high-quality optimization and code generation
support for many targets. For more general information, please see the
`Clang Web Site <http://clang.llvm37.org>`_ or the `LLVM37 Web
Site <http://llvm37.org>`_.
...
`Clang Static Analyzer <http://clang-analyzer.llvm37.org>`_, please see its web
page.
...
edge <http://llvm37.org/releases/download.html#svn>`_. Clang goes to great
...
`static analyzer <http://clang-analyzer.llvm37.org>`_ can also be
influenced by the user via changes to the source code. See the available
`annotations <http://clang-analyzer.llvm37.org/annotations.html>`_ and the
analyzer's `FAQ
page <http://clang-analyzer.llvm37.org/faq.html#exclude_code>`_ for more
information.
...
.. option:: -ftrap-function=[name]
   ...
   LLVM37 code generator translates ``__builtin_trap()`` to a trap
...
4. Sampling profiles must be generated by an external tool. The profile
   ...
   by LLVM37. The section on sampling profilers describes one of the supported
...
2. Run the executable under a sampling profiler. The specific profiler
   ...
   into the format that the LLVM37 optimizer understands. Currently, there
...
3. Convert the collected profile data to LLVM37's sample profile format.
   This is currently supported via the AutoFDO converter ``create_llvm37_prof``.
   It is available at http://github.com/google/autofdo. Once built and
   installed, you can convert the ``perf.data`` file to LLVM37 using
   the command:

   .. code-block:: console

     $ create_llvm37_prof --binary=./code --out=code.prof
...
read by the backend. LLVM37 supports three different sample profile formats:
...
2. Binary encoding. This uses a more efficient encoding that yields smaller
   ...
   generated from the text format using the ``llvm37-profdata`` tool.
...
3. GCC encoding. This is based on the gcov format, which is accepted by GCC. It
   ...
   to the binary encoding, it can be generated using the ``llvm37-profdata`` tool.

If you are using Linux Perf to generate sampling profiles, you can use the
conversion tool ``create_llvm37_prof`` described in the previous section.
...
of the other two, consult the ``ProfileData`` library in in LLVM37's source tree
(specifically, ``llvm37/lib/ProfileData/SampleProfWriter.cpp``).
...
3. Combine profiles from multiple runs and convert the "raw" profile format to
   ...
   ``llvm37-profdata`` tool to do this.

   .. code-block:: console

     $ llvm37-profdata merge -output=code.profdata code-*.profraw
...
-  clang does not support #pragma weak (`bug
   3679 <http://llvm37.org/bugs/show_bug.cgi?id=3679>`_). Due to the uses
...
-  clang does not support global register variables; this is unlikely to
   be implemented soon because it requires additional LLVM37 backend
   support.
...
the `bug
tracker <http://llvm37.org/bugs/buglist.cgi?quicksearch=product%3Aclang+component%3A-New%2BBugs%2CAST%2CBasic%2CDriver%2CHeaders%2CLLVM37%2BCodeGen%2Cparser%2Cpreprocessor%2CSemantic%2BAnalyzer>`_
...
<http://clang.llvm37.org/compatibility.html#dep_lookup_bases>`_, which is
...
both the clang support and the LLVM37 backend support are highly
experimental.
...
tree. This level of support is also sufficient for conversion to LLVM37 IR
for simple programs. Proper support for conversion to LLVM37 IR requires
adding code to ``lib/CodeGen/CGCall.cpp`` at the moment; this is likely to
change soon, though. Generating assembly requires a suitable LLVM37
backend.
...
`Some tests might fail <http://llvm37.org/bugs/show_bug.cgi?id=9072>`_ on
``x86_64-w64-mingw32``.
...
clang-cl can also be used from inside Visual Studio  by using an LLVM37 Platform
Toolset.
...
Please `file a bug <http://llvm37.org/bugs/enter_bug.cgi?product=clang&component=Driver>`_
...
Execute ``clang-cl /?`` to see a list of supported options:
  ...
  ::
    ...
    OPTIONS:
      ...
      -mllvm37 <value>          Additional arguments to forward to LLVM37's option processing

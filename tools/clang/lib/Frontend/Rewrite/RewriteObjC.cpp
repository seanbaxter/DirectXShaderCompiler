...
namespace {
  ...
  protected:
    ...
    llvm37::SmallPtrSet<ObjCInterfaceDecl*, 8> ObjCForwardDecls;
...
clang::CreateObjCRewriter(const std::string &InFile, raw_ostream *OS,
                          ...
                          bool SilenceRewriteMacroWarning) {
  return llvm37::make_unique<RewriteObjCFragileABI>(InFile, OS, Diags, LOpts,
                                                  SilenceRewriteMacroWarning);
}
...
Stmt *RewriteObjC::RewriteObjCSynchronizedStmt(ObjCAtSynchronizedStmt *S) {
  ...
  llvm37::raw_string_ostream syncExprBuf(syncExprBufS);
...
Stmt *RewriteObjC::RewriteObjCTryStmt(ObjCAtTryStmt *S) {
  ...
  for (unsigned I = 0, N = S->getNumCatchStmts(); I != N; ++I) {
    ...
    if (Catch->hasEllipsis()) {
      ...
      llvm37_unreachable("@catch rewrite bug");
    }
  }
...
void RewriteObjC::RewriteObjCInternalStruct(ObjCInterfaceDecl *CDecl,
  ...
  if (!ObjCSynthesizedStructs.insert(CDecl).second)
    llvm37_unreachable("struct already synthesize- SynthesizeObjCInternalStruct");
}

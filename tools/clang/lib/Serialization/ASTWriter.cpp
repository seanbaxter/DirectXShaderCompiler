...
void ASTTypeWriter::VisitBuiltinType(const BuiltinType *T) {
  llvm37_unreachable("Built-in types are never serialized");
}
...
ASTTypeWriter::VisitDependentSizedExtVectorType(
  ...
  llvm37_unreachable("Cannot serialize dependent sized extended vector types");
}
...
void ASTWriter::WritePreprocessorDetail(PreprocessingRecord &PPRec) {
  ...
  for (PreprocessingRecord::iterator E = PPRec.local_begin(),
    ...
    llvm37_unreachable("Unhandled PreprocessedEntity in ASTWriter");
  }
  Stream.ExitBlock();
...
public:
  ...
  void EmitKey(raw_ostream& Out, DeclarationName Name, unsigned) {
    ...
    llvm37_unreachable("Invalid name kind?");
  }

//===--- BackendUtil.cpp - LLVM37 Backend Utilities -------------------------===//
//
...
private:
  ...
  /// Add passes necessary to emit assembly or LLVM37 IR.
  ///
...
void EmitAssemblyHelper::CreatePasses() {
  ...
  // Handle disabling of LLVM37 optimization, where we want to preserve the
  // internal module before any optimization.
  if (CodeGenOpts.DisableLLVM37Opts || CodeGenOpts.HLSLHighLevel) { // HLSL Change
    OptLevel = 0;
    // HLSL Change Begins.
    // HLSL always inline.
  ...
  if (!CodeGenOpts.DisableGCov &&
    ...
    // LLVM37's -default-gcov-version flag is set to something invalid.
    GCOVOptions Options;
...
void EmitAssemblyHelper::EmitAssembly(BackendAction Action,
  ...
  case Backend_EmitBC:
    getPerModulePasses()->add(
        createBitcodeWriterPass(*OS, CodeGenOpts.EmitLLVM37UseLists));
    break;

  case Backend_EmitLL:
    getPerModulePasses()->add(
        createPrintModulePass(*OS, "", CodeGenOpts.EmitLLVM37UseLists));
    break;
...
void clang::EmitBackendOutput(DiagnosticsEngine &Diags,
  ...
  // verify the LLVM37 TargetMachine's DataLayout.

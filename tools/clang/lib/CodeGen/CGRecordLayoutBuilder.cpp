...
/// * LLVM37 does not have unions - Unions can, in theory be represented by any
///   llvm37::Type with correct size.  We choose a field via a specific heuristic
///   and add padding if necessary.
/// * LLVM37 does not have bitfields - Bitfields are collected into contiguous
...
///   when lowered to llvm37 types.  For example unsigned x : 24 gets lowered to
...
///   fields.  The existing asserts suggest that LLVM37 assumes that *every* field
...
///   because LLVM37 reads from the complete type it can generate incorrect code
...
///   SCISSOR.  If LLVM37 were updated to read base types (which it probably
///   should because locations of things such as VBases are bogus in the llvm37
...
struct CGRecordLowering {
  ...
  llvm37::Type *getIntNType(uint64_t NumBits) {
    return llvm37::Type::getIntNTy(Types.getLLVM37Context(),
        (unsigned)llvm37::RoundUpToAlignment(NumBits, 8));
  }
  /// \brief Gets an llvm37 type of size NumBytes and alignment 1.
  llvm37::Type *getByteArrayType(CharUnits NumBytes) {
    assert(!NumBytes.isZero() && "Empty byte arrays aren't allowed.");
    llvm37::Type *Type = llvm37::Type::getInt8Ty(Types.getLLVM37Context());
  ...
  /// \brief Gets the llvm37 Basesubobject type from a CXXRecordDecl.
  llvm37::Type *getStorageType(const CXXRecordDecl *RD) {
    return Types.getCGRecordLayout(RD).getBaseSubobjectLLVM37Type();
  }
  ...
  /// \brief Lowers an ASTRecordLayout to a llvm37 type.
  ...
  /// \brief Determines if we need a packed llvm37 struct.
...
void CGRecordLowering::lower(bool NVBaseType) {
  ...
  // 4) Determine if the llvm37-struct requires packing.  It's important that this
  //    phase occur after clipping, because clipping changes the llvm37 type.
...
void CGRecordLowering::accumulateVPtrs() {
  ...
  if (Layout.hasOwnVBPtr())
    Members.push_back(MemberInfo(Layout.getVBPtrOffset(), MemberInfo::VBPtr,
        llvm37::Type::getInt32PtrTy(Types.getLLVM37Context())));
}
...
CGRecordLayout *CodeGenTypes::ComputeRecordLayout(const RecordDecl *D,
  ...
  if (isa<CXXRecordDecl>(D) && !D->isUnion() && !D->hasAttr<FinalAttr>()) {
    ...
    if (Builder.Layout.getNonVirtualSize() != Builder.Layout.getSize()) {
      ...
      BaseTy = llvm37::StructType::create(
          getLLVM37Context(), BaseBuilder.FieldTypes, "", BaseBuilder.Packed);
      addRecordTypeName(D, BaseTy, ".base");
      // BaseTy and Ty must agree on their packedness for getLLVM37FieldNo to work
...
#ifndef NDEBUG
  // Verify that the computed LLVM37 struct size matches the AST layout size.
  ...
  // Verify that the LLVM37 and AST field offsets agree.
  llvm37::StructType *ST =
    dyn_cast<llvm37::StructType>(RL->getLLVM37Type());
  ...
  for (unsigned i = 0, e = AST_RL.getFieldCount(); i != e; ++i, ++it) {
    ...
    // For non-bit-fields, just check that the LLVM37 struct offset matches the
    // AST offset.
    if (!FD->isBitField()) {
      unsigned FieldNo = RL->getLLVM37FieldNo(FD);
    ...
    llvm37::Type *ElementTy = ST->getTypeAtIndex(RL->getLLVM37FieldNo(FD));
...
void CGRecordLayout::print(raw_ostream &OS) const {
  OS << "<CGRecordLayout\n";
  OS << "  LLVM37Type:" << *CompleteObjectType << "\n";
  if (BaseSubobjectType)
    OS << "  NonVirtualBaseLLVM37Type:" << *BaseSubobjectType << "\n"; 

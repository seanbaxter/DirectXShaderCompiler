...
static void computeBlockInfo(CodeGenModule &CGM, CodeGenFunction *CGF,
  ...
  if (!block->hasCaptures()) {
    info.StructureType =
      llvm37::StructType::get(CGM.getLLVMContext(), elementTypes, true);
  ...
  if (block->capturesCXXThis()) {
    ...
    llvm37::Type *llvmType = CGM.getTypes().ConvertType(thisType);
  ...
  for (const auto &CI : block->captures()) {
    ...
    if (CI.isByRef()) {
      ...
      llvm37::Type *llvmType = CGM.getTypes().ConvertType(byRefPtrTy);
      std::pair<CharUnits,CharUnits> tinfo
...
CodeGenFunction::GenerateBlockFunction(GlobalDecl GD,
  ...
  llvm37::FunctionType *fnLLVMType = CGM.getTypes().GetFunctionType(fnInfo);

  StringRef name = CGM.getBlockMangledName(GD, blockDecl);
  llvm37::Function *fn = llvm37::Function::Create(
      fnLLVMType, llvm37::GlobalValue::InternalLinkage, name, &CGM.getModule());
...
llvm37::Type *CodeGenFunction::BuildByRefType(const VarDecl *D) {
  ...
  llvm37::StructType *ByRefType =
    llvm37::StructType::create(getLLVMContext(),

...
uint32_t EmitTypeHandler::getOrCreateConstant(SpirvConstant *inst) {
  ...
  llvm37_unreachable("cannot emit unknown constant type");
}
...
uint32_t EmitTypeHandler::emitType(const SpirvType *type) {
  ...
  // that is clearly a bug.
    ...
    llvm37_unreachable("found hybrid type when emitting SPIR-V");
  }
  // Unhandled types
  else {
    llvm37_unreachable("unhandled type in emitType");
  }

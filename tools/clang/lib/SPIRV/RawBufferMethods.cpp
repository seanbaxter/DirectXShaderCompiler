...
SpirvInstruction *RawBufferHandler::load64BitsAtBitOffset0(
  ...
  auto *constUint1 =
      spvBuilder.getConstantInt(astContext.UnsignedIntTy, llvm37::APInt(32, 1));
  auto *constUint32 =
      spvBuilder.getConstantInt(astContext.UnsignedIntTy, llvm37::APInt(32, 32));
...
SpirvInstruction *RawBufferHandler::processTemplatedLoadFromBuffer(
  ...
  // Vector types
  {
    ...
    if (isVectorType(targetType, &elemType, &elemCount)) {
      llvm37::SmallVector<SpirvInstruction *, 4> loadedElems;
  ...
  // Matrix types
  {
    ...
    if (isMxNMatrix(targetType, &elemType, &numRows, &numCols)) {
      llvm37::SmallVector<SpirvInstruction *, 4> loadedElems;
      llvm37::SmallVector<SpirvInstruction *, 4> loadedRows;
  ...
  llvm37_unreachable("templated buffer load unimplemented for type");
}
...
QualType RawBufferHandler::serializeToScalarsOrStruct(
  ...
  llvm37_unreachable("unhandled type when serializing an array");
}
...
void RawBufferHandler::processTemplatedStoreToBuffer(SpirvInstruction *value,
  ...
  llvm37_unreachable("templated buffer store unimplemented for type");
}

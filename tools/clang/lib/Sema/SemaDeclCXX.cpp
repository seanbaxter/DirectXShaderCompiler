...
static unsigned getRecordDiagFromTagKind(TagTypeKind Tag) {
  ...
  default: llvm37_unreachable("Invalid tag kind for record diagnostic!");
  }
}
...
typedef llvm37::SmallPtrSet<QualType, 4> IndirectBaseSet;
...
// HLSL Change Starts
static bool isConcreteBaseType(const QualType& t)
{
  // Other than interfaces, all base types are concrete.
  const Type* type = t.getTypePtr();
  if (!type->isRecordType()) return true; // Likely should have been rejected before.
  return !(type->getAsCXXRecordDecl()->getTagKind() == TagTypeKind::TTK_Interface);
}
// HLSL Change Ends
...
bool Sema::AttachBaseSpecifiers(CXXRecordDecl *Class, CXXBaseSpecifier **Bases,
 ...
 if (NumBases == 0)
  ...
  // HLSL Change Starts
  if (Class->getTagKind() == TagTypeKind::TTK_Interface) {
    Diag(Bases[0]->getLocStart(), diag::err_hlsl_interfaces_cannot_inherit);
    return true;
  }
  // HLSL Change Ends
  ...
  bool ConcreteBaseTypeFound = false; // HLSL Change
  bool ConcreteBaseTypeReported = false; // HLSL Change
  ...
  for (unsigned idx = 0; idx < NumBases; ++idx) {
    ...
    // HLSL Change Starts
    if (!ConcreteBaseTypeReported) {
      if (isConcreteBaseType(NewBaseType)) {
        if (ConcreteBaseTypeFound) {
          Diag(Bases[idx]->getLocStart(), diag::err_hlsl_multiple_concrete_bases);
          Invalid = true;
          ConcreteBaseTypeReported = true;
        } else {
          ConcreteBaseTypeFound = true;
        }
      }
    }
    // HLSL Change Ends
...
namespace {
  ...
  static void DiagnoseUninitializedFields(
    ...
    llvm37::SmallPtrSet<ValueDecl*, 4> UninitializedFields;
    ...
    llvm37::SmallPtrSet<QualType, 4> UninitializedBaseClasses;
    for (auto I : RD->bases())
    ...
    if (!TyD) {
      ...
      if (R.empty() && BaseType.isNull() &&
          (Corr = CorrectTypo(
               ...
               llvm37::make_unique<MemInitializerValidatorCCC>(ClassDecl),
...
struct BaseAndFieldInfo {
  ...
  llvm37::DenseMap<const void *, CXXCtorInitializer*> AllBaseFields;
  SmallVector<CXXCtorInitializer*, 8> AllToInit;
  llvm37::DenseMap<TagDecl*, FieldDecl*> ActiveUnionMember;
  ...
  bool isImplicitCopyOrMove() const {
    ...
    llvm37_unreachable("Invalid ImplicitInitializerKind!");
  }
...
bool Sema::SetCtorInitializers(CXXConstructorDecl *Constructor, bool AnyErrors,
  ...
  llvm37::SmallPtrSet<CXXBaseSpecifier *, 16> DirectVBases;
...
typedef llvm37::DenseMap<RecordDecl*, UnionEntry> RedundantUnionMap;
...
void Sema::ActOnMemInitializers(Decl *ConstructorDecl,
  ...
  llvm37::DenseMap<const void *, CXXCtorInitializer *> Members;
...
Sema::MarkBaseAndMemberDestructorsReferenced(SourceLocation Location,
  ...
  llvm37::SmallPtrSet<const RecordType *, 8> DirectVirtualBases;
...
void Sema::DiagnoseAbstractType(const CXXRecordDecl *RD) {
  ...
  // more than once.
  llvm37::SmallPtrSet<const CXXMethodDecl *, 8> SeenPureMethods;
...
static FunctionProtoType::ExtProtoInfo getImplicitMethodEPI(Sema &S,
  ...
  EPI.ExceptionSpec.Type = EST_BasicNoexcept; // HLSL Change - EST_Unevaluated to EST_BasicNoexcept
...
void Sema::CheckExplicitlyDefaultedSpecialMember(CXXMethodDecl *MD) {
  ...
  if (First) {
    ...
    MD->setType(Context.getFunctionType(ReturnType,
                                        llvm37::makeArrayRef(&ArgType,
                                                           ExpectedParams),
                                        EPI, None)); // HLSL Change - special members are all-in params
  }
...
void Sema::CheckExplicitlyDefaultedMemberExceptionSpec(
  ...
  const FunctionProtoType *ImplicitType = cast<FunctionProtoType>(
    Context.getFunctionType(Context.VoidTy, None, EPI, None)); // HLSL Change - special members are all-in params
...
struct SpecialMemberDeletionInfo {
  ...
  SpecialMemberDeletionInfo(Sema &S, CXXMethodDecl *MD,
    ...
    switch (CSM) {
      ...
      case Sema::CXXDestructor:
        break;
      case Sema::CXXInvalid:
        llvm37_unreachable("invalid special member kind");
    }
  ...
  typedef llvm37::PointerUnion<CXXBaseSpecifier*, FieldDecl*> Subobject;
...
static bool findTrivialSpecialMember(Sema &S, CXXRecordDecl *RD,
  ...
  switch (CSM) {
  case Sema::CXXInvalid:
    llvm37_unreachable("not a special member");
  ...
  llvm37_unreachable("unknown special method kind");
}
...
bool Sema::SpecialMemberIsTrivial(CXXMethodDecl *MD, CXXSpecialMember CSM,
  ...
  if (CSM != CXXDestructor && MD->getParent()->isDynamicClass()) {
    ...
    llvm37_unreachable("dynamic class with no vbases and no virtual functions");
  }
...
namespace {
  struct FindHiddenVirtualMethodData {
    ...
    llvm37::SmallPtrSet<const CXXMethodDecl *, 8> OverridenAndUsingBaseMethods;
    SmallVector<CXXMethodDecl *, 8> OverloadedMethods;
  };
}
...
static bool CheckMostOverridenMethods(const CXXMethodDecl *MD,
                  const llvm37::SmallPtrSetImpl<const CXXMethodDecl *>& Methods) {
  if (MD->size_overridden_methods() == 0)
...
static void AddMostOverridenMethods(const CXXMethodDecl *MD,
                        llvm37::SmallPtrSetImpl<const CXXMethodDecl *>& Methods) {
  if (MD->size_overridden_methods() == 0)
...
void Sema::ActOnFinishCXXMemberSpecification(Scope* S, SourceLocation RLoc,
  ...
  ActOnFields(S, RLoc, TagDecl, llvm37::makeArrayRef(
...
QualType Sema::CheckConstructorDeclarator(Declarator &D, QualType R,
  ...
  return Context.getFunctionType(Context.VoidTy, Proto->getParamTypes(), EPI, None); // HLSL Change - constructors members are all-in params
}
...
QualType Sema::CheckDestructorDeclarator(Declarator &D, QualType R,
  ...
  EPI.TypeQuals = 0;
  EPI.RefQualifier = RQ_None;
  return Context.getFunctionType(Context.VoidTy, None, EPI, None); // HLSL Change - no destructor args
}
...
static bool TryNamespaceTypoCorrection(Sema &S, LookupResult &R, Scope *Sc,
  ...
  if (TypoCorrection Corrected =
          S.CorrectTypo(R.getLookupNameInfo(), R.getLookupKind(), Sc, &SS,
                        llvm37::make_unique<NamespaceValidatorCCC>(),
...
NamedDecl *Sema::BuildUsingDeclaration(Scope *S, AccessSpecifier AS,
  ...
  if (R.empty() &&
    ...
    if (TypoCorrection Corrected = CorrectTypo(
            R.getLookupNameInfo(), R.getLookupKind(), S, &SS,
            llvm37::make_unique<UsingValidatorCCC>(
...
bool Sema::CheckUsingDeclQualifier(SourceLocation UsingLoc,
  ...
  struct UserData {
    llvm37::SmallPtrSet<const CXXRecordDecl*, 4> Bases;
...
CXXConstructorDecl *Sema::DeclareImplicitDefaultConstructor(
  ...
  DefaultCon->setType(Context.getFunctionType(Context.VoidTy, None, EPI, None)); // HLSL Change - add param mods
...
private:
      ...
      do
        declareCtor(UsingLoc, Ctor,
                    SemaRef.Context.getFunctionType(
                        Ctor->getReturnType(), ArgTypes.slice(0, Params), EPI, None)); // HLSL Change - add param mods
  ...
  void declareCtor(SourceLocation UsingLoc, const CXXConstructorDecl *BaseCtor,
    ...
    DerivedCtor->setType(Context.getFunctionType(FPT->getReturnType(),
                                                 FPT->getParamTypes(), EPI, None)); // HLSL Change - add param mods
  ...
  typedef llvm37::DenseMap<const Type *, InheritingConstructorsForType> MapType;
  MapType Map;
};
}
...
CXXDestructorDecl *Sema::DeclareImplicitDestructor(CXXRecordDecl *ClassDecl) {
  ...
  Destructor->setType(Context.getFunctionType(Context.VoidTy, None, EPI, None)); // HLSL Change
...
void Sema::AdjustDestructorExceptionSpec(CXXRecordDecl *ClassDecl,
  ...
  Destructor->setType(Context.getFunctionType(Context.VoidTy, None, EPI, None)); // HLSL Change - add param mods
...
buildMemcpyForAssignmentOp(Sema &S, SourceLocation Loc, QualType T,
  ...
  llvm37::APInt Size(S.Context.getTypeSize(SizeType),
...
buildSingleCopyAssignRecursively(Sema &S, SourceLocation Loc, QualType T,
  ...
  llvm37::APInt Zero(S.Context.getTypeSize(SizeType), 0);
...
CXXMethodDecl *Sema::DeclareImplicitCopyAssignment(CXXRecordDecl *ClassDecl) {
  ...
  CopyAssignment->setType(Context.getFunctionType(RetType, ArgType, EPI, None)); // HLSL Change - add param mods
...
CXXMethodDecl *Sema::DeclareImplicitMoveAssignment(CXXRecordDecl *ClassDecl) {
  ...
  MoveAssignment->setType(Context.getFunctionType(RetType, ArgType, EPI, None)); // HLSL Change - add param mods
...
static void checkMoveAssignmentForRepeatedMove(Sema &S, CXXRecordDecl *Class,
  ...
  llvm37::SmallVector<CXXBaseSpecifier *, 16> Worklist;
  typedef llvm37::DenseMap<CXXRecordDecl*, CXXBaseSpecifier*> VBaseMap;
  VBaseMap VBases;
...
CXXConstructorDecl *Sema::DeclareImplicitCopyConstructor(
  ...
  CopyConstructor->setType(
      Context.getFunctionType(Context.VoidTy, ArgType, EPI, None)); // HLSL Change - add param mods
...
CXXConstructorDecl *Sema::DeclareImplicitMoveConstructor(
  ...
  MoveConstructor->setType(
      Context.getFunctionType(Context.VoidTy, ArgType, EPI, None)); // HLSL Change - all in-args
...
Sema::CompleteConstructorCall(CXXConstructorDecl *Constructor,
  ...
  CheckConstructorCall(Constructor,
                       llvm37::makeArrayRef(AllArgs.data(), AllArgs.size()),
                       Proto, Loc);
...
Decl *Sema::BuildStaticAssertDeclaration(SourceLocation StaticAssertLoc,
  ...
  if (!AssertExpr->isTypeDependent() && !AssertExpr->isValueDependent() &&
    ...
    if (!Failed && !Cond) {
      ...
      llvm37::raw_svector_ostream Msg(MsgBuffer);
      if (AssertMessage)
...
void Sema::SetDeclDefaulted(Decl *Dcl, SourceLocation DefaultLoc) {
  ...
  if (MD) {
    ...
    case CXXMoveAssignment:
      ...
      break;
    case CXXInvalid:
      llvm37_unreachable("Invalid special member.");
    }
  } else {
    Diag(DefaultLoc, diag::err_default_special_members);
  }
...
void Sema::LoadExternalVTableUses() {
  ...
  for (unsigned I = 0, N = VTables.size(); I != N; ++I) {
    llvm37::DenseMap<CXXRecordDecl *, bool>::iterator Pos
...
void Sema::MarkVTableUsed(SourceLocation Loc, CXXRecordDecl *Class,
  ...
  std::pair<llvm37::DenseMap<CXXRecordDecl *, bool>::iterator, bool>
...
void DelegatingCycleHelper(CXXConstructorDecl* Ctor,
                           llvm37::SmallSet<CXXConstructorDecl*, 4> &Valid,
                           llvm37::SmallSet<CXXConstructorDecl*, 4> &Invalid,
                           llvm37::SmallSet<CXXConstructorDecl*, 4> &Current,
                           Sema &S) {
...
void Sema::CheckDelegatingCtorCycles() {
  llvm37::SmallSet<CXXConstructorDecl*, 4> Valid, Invalid, Current;
  ...
  for (llvm37::SmallSet<CXXConstructorDecl *, 4>::iterator CI = Invalid.begin(),
                                                         CE = Invalid.end();
       CI != CE; ++CI)
    (*CI)->setInvalidDecl();
}
...
bool Sema::checkThisInStaticMemberFunctionAttributes(CXXMethodDecl *Method) {
  ...
  for (const auto *A : Method->attrs()) {
    ...
    if (const auto *G = dyn_cast<GuardedByAttr>(A))
      ...
      Args = llvm37::makeArrayRef(AA->args_begin(), AA->args_size());
    else if (const auto *AB = dyn_cast<AcquiredBeforeAttr>(A))
      Args = llvm37::makeArrayRef(AB->args_begin(), AB->args_size());
    else if (const auto *ETLF = dyn_cast<ExclusiveTrylockFunctionAttr>(A)) {
      Arg = ETLF->getSuccessValue();
      Args = llvm37::makeArrayRef(ETLF->args_begin(), ETLF->args_size());
    } else if (const auto *STLF = dyn_cast<SharedTrylockFunctionAttr>(A)) {
      Arg = STLF->getSuccessValue();
      Args = llvm37::makeArrayRef(STLF->args_begin(), STLF->args_size());
    } else if (const auto *LR = dyn_cast<LockReturnedAttr>(A))
      Arg = LR->getArg();
    else if (const auto *LE = dyn_cast<LocksExcludedAttr>(A))
      Args = llvm37::makeArrayRef(LE->args_begin(), LE->args_size());
    else if (const auto *RC = dyn_cast<RequiresCapabilityAttr>(A))
      Args = llvm37::makeArrayRef(RC->args_begin(), RC->args_size());
    else if (const auto *AC = dyn_cast<AcquireCapabilityAttr>(A))
      Args = llvm37::makeArrayRef(AC->args_begin(), AC->args_size());
...
void Sema::checkExceptionSpecification(
  ...
  // HLSL Change Starts
  if (LangOpts.HLSL) {
    assert(DynamicExceptions.size() == 0);
    assert(DynamicExceptionRanges.size() == 0);
    assert(NoexceptExpr == nullptr);
...
void Sema::actOnDelayedExceptionSpecification(Decl *MethodD,
  ...
  // Check the exception specification.
  llvm37::SmallVector<QualType, 4> Exceptions;
  FunctionProtoType::ExceptionSpecInfo ESI;

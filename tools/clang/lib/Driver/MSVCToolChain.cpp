...
bool MSVCToolChain::IsUnwindTablesDefault() const {
  ...
  // such as ARM and PPC actually require unwind tables, but LLVM37 doesn't know
...
static bool readFullStringValue(HKEY hkey, const char *valueName,
  ...
  // from LLVM37 though, so in order to do this we will probably need to create
  // a registry abstraction in LLVM37Support that is Windows only.
  DWORD result = 0;
  DWORD valueSize = 0;
  DWORD type = 0;

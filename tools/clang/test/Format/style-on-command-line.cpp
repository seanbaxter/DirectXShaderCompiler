...
// RUN: clang-format -style="{BasedOnStyle: LLVM37, IndentWidth: 7}" %t.cpp | FileCheck -strict-whitespace -check-prefix=CHECK2 %s
// RUN: clang-format -style="{BasedOnStyle: invalid, IndentWidth: 7}" -fallback-style=LLVM37 %t.cpp 2>&1 | FileCheck -strict-whitespace -check-prefix=CHECK3 %s
// RUN: clang-format -style="{lsjd}" %t.cpp -fallback-style=LLVM37 2>&1 | FileCheck -strict-whitespace -check-prefix=CHECK4 %s
...
// RUN: clang-format -style="{BasedOnStyle: LLVM37, PointerBindsToType: true}" %t.cpp | FileCheck -strict-whitespace -check-prefix=CHECK8 %s
...
// CHECK3: Error parsing -style: {{I|i}}nvalid argument, using LLVM37 style
// CHECK3: {{^  int \*i;$}}
// CHECK4: Error parsing -style: {{I|i}}nvalid argument, using LLVM37 style
// CHECK4: {{^  int \*i;$}}
// CHECK5: {{^     int\* i;$}}

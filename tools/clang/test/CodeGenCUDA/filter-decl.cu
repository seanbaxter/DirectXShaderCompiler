// RUN: %clang_cc1 -triple %itanium_abi_triple -emit-llvm37 %s -o - | FileCheck -check-prefix=CHECK-HOST %s
// RUN: %clang_cc1 -triple %itanium_abi_triple -emit-llvm37 %s -o - -fcuda-is-device | FileCheck -check-prefix=CHECK-DEVICE %s

...
TEST_F(SpirvConstantTest, Uint16) {
  clang::ASTContext &astContext = getAstContext();
  const auto u16 = llvm37::APInt(16, 12u);
...
TEST_F(SpirvConstantTest, Int16) {
  clang::ASTContext &astContext = getAstContext();
  const auto i16 = llvm37::APInt(16, -12, /*isSigned*/ true);
...
TEST_F(SpirvConstantTest, Uint32) {
  clang::ASTContext &astContext = getAstContext();
  const auto u32 = llvm37::APInt(32, 65536);
...
TEST_F(SpirvConstantTest, Int32) {
  clang::ASTContext &astContext = getAstContext();
  const auto i32 = llvm37::APInt(32, -65536, /*isSigned*/ true);
...
TEST_F(SpirvConstantTest, Uint64) {
  clang::ASTContext &astContext = getAstContext();
  const auto u64 = llvm37::APInt(64, 4294967296);
...
TEST_F(SpirvConstantTest, Int64) {
  clang::ASTContext &astContext = getAstContext();
  const auto i64 = llvm37::APInt(64, -4294967296, /*isSigned*/ true);
...
TEST_F(SpirvConstantTest, Float32) {
  clang::ASTContext &astContext = getAstContext();
  const auto f32 = llvm37::APFloat(1.5f);
...
TEST_F(SpirvConstantTest, CheckOperatorEqualOnInt2) {
  clang::ASTContext &astContext = getAstContext();
  const auto i1 = llvm37::APInt(32, 5, /*isSigned*/ true);
  const auto i2 = llvm37::APInt(32, 7, /*isSigned*/ true);
...
TEST_F(SpirvConstantTest, CheckOperatorEqualOnFloat2) {
  clang::ASTContext &astContext = getAstContext();
  const auto f1 = llvm37::APFloat(1.5f);
  const auto f2 = llvm37::APFloat(1.6f);
...
TEST_F(SpirvConstantTest, CheckOperatorEqualOnInt3) {
  ...
  clang::ASTContext &astContext = getAstContext();
  const auto i32 = llvm37::APInt(32, 7, /*isSigned*/ true);

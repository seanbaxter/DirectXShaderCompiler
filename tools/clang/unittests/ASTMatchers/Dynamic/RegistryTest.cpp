...
public:
  ...
  llvm37::Optional<MatcherCtor> lookupMatcherCtor(StringRef MatcherName) {
  ...
  VariantMatcher constructMatcher(StringRef MatcherName,
    ...
    llvm37::Optional<MatcherCtor> Ctor = lookupMatcherCtor(MatcherName);
    VariantMatcher Out;
    if (Ctor)
  ...
  CompVector getCompletions(StringRef MatcherName1, unsigned ArgNo1) {
    std::vector<std::pair<MatcherCtor, unsigned> > Context;
    llvm37::Optional<MatcherCtor> Ctor = lookupMatcherCtor(MatcherName1);
    if (!Ctor)
      return CompVector();

...
static void VerifyPdbUtil(dxc::DxcDllSupport &dllSupport,
    ...
    llvm37::ArrayRef<std::pair<const WCHAR *, const WCHAR *> > ExpectedArgs,
    llvm37::ArrayRef<std::pair<const WCHAR *, const WCHAR *> > ExpectedFlags,
    llvm37::ArrayRef<const WCHAR *> ExpectedDefines,
    ...
    const std::string &IncludedFile)
{
  ...
  auto TestArgumentPair = [](llvm37::ArrayRef<std::wstring> Args, llvm37::ArrayRef<std::pair<const WCHAR *, const WCHAR *> > Expected) {
    for (size_t i = 0; i < Expected.size(); i++) {

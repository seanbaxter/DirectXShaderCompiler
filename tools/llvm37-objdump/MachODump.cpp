...
//===----------------------------------------------------------------------===//
//
// This file implements the MachO-specific dumper for llvm37-objdump.
//
//===----------------------------------------------------------------------===//

#include "llvm37-objdump.h"
...
static const Target *GetTarget(const MachOObjectFile *MachOObj,
  ...
  *ThumbTarget = TargetRegistry::lookupTarget(ThumbTripleName, Error);
  ...
  errs() << "llvm37-objdump: error: unable to get target for '";
  if (!TheTarget)
    errs() << TripleName;
  else
...
static bool checkMachOAndArchFlags(ObjectFile *O, StringRef Filename) {
  if (isa<MachOObjectFile>(O) && !ArchAll && ArchFlags.size() != 0) {
    ...
    if (!ArchFound) {
      errs() << "llvm37-objdump: file: " + Filename + " does not contain "
...
void llvm37::ParseInputMachO(StringRef Filename) {
  ...
  for (unsigned i = 0; i < ArchFlags.size(); ++i) {
    if (ArchFlags[i] == "all") {
      ...
      if (!MachOObjectFile::isValidArch(ArchFlags[i])) {
        errs() << "llvm37-objdump: Unknown architecture named '" + ArchFlags[i] +
                      "'for the -arch option\n";
        return;
      }
    }
  ...
  if (std::error_code EC = BinaryOrErr.getError()) {
    errs() << "llvm37-objdump: '" << Filename << "': " << EC.message() << ".\n";
    return;
  }
...
// LLVM37OpInfo1 struct defined in the header "llvm37-c/Disassembler.h" as symbol
// names and addends of the symbolic expression to add for the operand.  The
// value of TagType is currently 1 (for the LLVM37OpInfo1 struct). If symbolic
...
static int SymbolizerGetOpInfo(void *DisInfo, uint64_t Pc, uint64_t Offset,
  ...
  struct LLVM37OpInfo1 *op_info = (struct LLVM37OpInfo1 *)TagBuf;
  uint64_t value = op_info->Value;

  // Make sure all fields returned are zero if we don't set them.
  memset((void *)op_info, '\0', sizeof(struct LLVM37OpInfo1));
  op_info->Value = value;
    ...
    if (isExtern) {
      ...
      case MachO::ARM_RELOC_HALF:
        if ((r_length & 0x1) == 1) {
          ...
          op_info->VariantKind = LLVM37Disassembler_VariantKind_ARM_HI16;
        } else {
          op_info->Value = other_half << 16 | value;
          op_info->VariantKind = LLVM37Disassembler_VariantKind_ARM_LO16;
        }
        break;
      default:
        break;
  ...
  if (Arch == Triple::aarch64) {
    ...
    case MachO::ARM64_RELOC_PAGE21:
      /* @page */
      op_info->VariantKind = LLVM37Disassembler_VariantKind_ARM64_PAGE;
      break;
    case MachO::ARM64_RELOC_PAGEOFF12:
      /* @pageoff */
      op_info->VariantKind = LLVM37Disassembler_VariantKind_ARM64_PAGEOFF;
      break;
    case MachO::ARM64_RELOC_GOT_LOAD_PAGE21:
      /* @gotpage */
      op_info->VariantKind = LLVM37Disassembler_VariantKind_ARM64_GOTPAGE;
      break;
    case MachO::ARM64_RELOC_GOT_LOAD_PAGEOFF12:
      /* @gotpageoff */
      op_info->VariantKind = LLVM37Disassembler_VariantKind_ARM64_GOTPAGEOFF;
      break;
    case MachO::ARM64_RELOC_TLVP_LOAD_PAGE21:
      /* @tvlppage is not implemented in llvm37-mc */
      op_info->VariantKind = LLVM37Disassembler_VariantKind_ARM64_TLVP;
      break;
    case MachO::ARM64_RELOC_TLVP_LOAD_PAGEOFF12:
      /* @tvlppageoff is not implemented in llvm37-mc */
      op_info->VariantKind = LLVM37Disassembler_VariantKind_ARM64_TLVOFF;
      break;
    default:
    case MachO::ARM64_RELOC_BRANCH26:
      op_info->VariantKind = LLVM37Disassembler_VariantKind_None;
      break;
    }
    return 1;
  }
...
// LLVM37Disassembler_ReferenceType_Out_Objc_Message.  If this not a method call
      ...
      if (info->selector_name != nullptr) {
        ...
        if (info->class_name != nullptr) {
          ...
          if (info->method != nullptr) {
            ...
            *ReferenceType = LLVM37Disassembler_ReferenceType_Out_Objc_Message;
          }
        } else {
...
// public defintions.  So the are here not in include/llvm37/Support/MachO.h .
...
// LLVM37Disassembler_ReferenceType_Out_LitPool_SymAddr .
//
...
static const char *GuessLiteralPointer(uint64_t ReferenceValue,
  ...
  if (classref && pointer_value == 0) {
    ...
    if (name != nullptr) {
      *ReferenceType = LLVM37Disassembler_ReferenceType_Out_Objc_Class_Ref;
  ...
  if (cfstring) {
    *ReferenceType = LLVM37Disassembler_ReferenceType_Out_Objc_CFString_Ref;
  ...
  if (name) {
    if (pointer_value != 0 && selref) {
      *ReferenceType = LLVM37Disassembler_ReferenceType_Out_Objc_Selector_Ref;
      info->selector_name = name;
    } else if (pointer_value != 0 && msgref) {
      info->class_name = nullptr;
      *ReferenceType = LLVM37Disassembler_ReferenceType_Out_Objc_Message_Ref;
      info->selector_name = name;
    } else
      *ReferenceType = LLVM37Disassembler_ReferenceType_Out_LitPool_CstrAddr;
    return name;
  }
  ...
  if (name) {
    *ReferenceType = LLVM37Disassembler_ReferenceType_Out_LitPool_SymAddr;
    return name;
  }
...
// "llvm37-c/Disassembler.h".  On return the ReferenceType can set to a specific
...
// LLVM37Disassembler_ReferenceType_DeMangled_Name .
#endif
//
// When this is called to get a symbol name for a branch target then the
// ReferenceType will be LLVM37Disassembler_ReferenceType_In_Branch and then
...
// set to LLVM37Disassembler_ReferenceType_Out_SymbolStub.
//
// When this is called with an value loaded via a PC relative load then
// ReferenceType will be LLVM37Disassembler_ReferenceType_In_PCrel_Load then the
...
static const char *SymbolizerSymbolLookUp(void *DisInfo,
  ...
  if (*ReferenceType == LLVM37Disassembler_ReferenceType_In_Branch) {
    *ReferenceName = GuessIndirectSymbol(ReferenceValue, info);
    if (*ReferenceName != nullptr) {
      method_reference(info, ReferenceType, ReferenceName);
      if (*ReferenceType != LLVM37Disassembler_ReferenceType_Out_Objc_Message)
        *ReferenceType = LLVM37Disassembler_ReferenceType_Out_SymbolStub;
    } else
#if HAVE_CXXABI_H
      ...
      if (info->demangled_name != nullptr) {
        ...
        *ReferenceType = LLVM37Disassembler_ReferenceType_DeMangled_Name;
      } else
        *ReferenceType = LLVM37Disassembler_ReferenceType_InOut_None;
    } else
#endif
      *ReferenceType = LLVM37Disassembler_ReferenceType_InOut_None;
  } else if (*ReferenceType == LLVM37Disassembler_ReferenceType_In_PCrel_Load) {
    *ReferenceName =
    ...
    // for use later if we see and add immediate instruction.
             ...
             *ReferenceType == LLVM37Disassembler_ReferenceType_In_ARM64_ADRP) {
    ...
    *ReferenceType = LLVM37Disassembler_ReferenceType_InOut_None;
    ...
    // passed in ReferenceValue.
             ...
             *ReferenceType == LLVM37Disassembler_ReferenceType_In_ARM64_ADDXri &&
    ...
    *ReferenceName =
    ...
    // instruction is passed in ReferenceValue.
             ...
             *ReferenceType == LLVM37Disassembler_ReferenceType_In_ARM64_LDRXui &&
             ReferencePC - 4 == info->adrp_addr &&
             (info->adrp_inst & 0x9f000000) == 0x90000000 &&
             (info->adrp_inst & 0x1f) == ((ReferenceValue >> 5) & 0x1f)) {
  ...
  // ReferenceValue is the PC plus the immediate value.
           ...
           (*ReferenceType == LLVM37Disassembler_ReferenceType_In_ARM64_LDRXl ||
            *ReferenceType == LLVM37Disassembler_ReferenceType_In_ARM64_ADR)) {
    *ReferenceName =
        GuessLiteralPointer(ReferenceValue, ReferencePC, ReferenceType, info);
    if (*ReferenceName == nullptr)
      *ReferenceType = LLVM37Disassembler_ReferenceType_InOut_None;
      ...
      for (uint64_t Index = Start; Index < End; Index += Size) {
        ...
        if (gotInst) {
          ...
          if (Arch == Triple::x86_64 || Arch == Triple::x86) {
            ...
            errs() << "llvm37-objdump: warning: invalid instruction encoding\n";
            if (Size == 0)
...
printMachOCompactUnwindSection(const MachOObjectFile *Obj,
  ...
  for (const RelocationRef &Reloc : CompactUnwind.relocations()) {
    ...
    if (OffsetInEntry == 0)
      ...
      llvm37_unreachable("Unexpected relocation in __compact_unwind section");
  }
...
static void printMachOUnwindInfoSection(const MachOObjectFile *Obj,
  ...
  for (unsigned i = 0; i < IndexEntries.size() - 1; ++i) {
    ...
    if (Kind == 2)
      ...
      llvm37_unreachable("Do not know how to print this kind of 2nd level page");
  }
}
...
void llvm37::printMachOUnwindInfo(const MachOObjectFile *Obj) {
  ...
  for (const SectionRef &Section : Obj->sections()) {
    ...
    if (SectName == "__compact_unwind")
      ...
      outs() << "llvm37-objdump: warning: unhandled __eh_frame section\n";
  }
}
...
StringRef SegInfo::segmentName(uint32_t SegIndex) {
  ...
  llvm37_unreachable("invalid segIndex");
}
...
const SegInfo::SectionInfo &SegInfo::findSection(uint32_t SegIndex,
  ...
  llvm37_unreachable("segIndex and offset not in any section");
}

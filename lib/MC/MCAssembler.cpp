...
const MCSymbol *MCAsmLayout::getBaseSymbol(const MCSymbol &Symbol) const {
  ...
  if (!Expr->evaluateAsValue(Value, *this))
    llvm37_unreachable("Invalid Expression");
...
uint64_t MCAssembler::computeFragmentSize(const MCAsmLayout &Layout,
  ...
  llvm37_unreachable("invalid fragment kind");
}
...
static void writeFragment(const MCAssembler &Asm, const MCAsmLayout &Layout,
  ...
  case MCFragment::FT_Align: {
    ...
    for (uint64_t i = 0; i != Count; ++i) {
      ...
      default: llvm37_unreachable("Invalid size!");
...
void MCAssembler::writeSectionData(const MCSection *Sec,
  ...
  if (Sec->isVirtualSection()) {
    ...
    for (MCSection::const_iterator it = Sec->begin(), ie = Sec->end(); it != ie;
         ++it) {
      switch (it->getKind()) {
      default: llvm37_unreachable("Invalid fragment in virtual section!");
      case MCFragment::FT_Data: {
  ...
  for (MCAssembler::iterator it = begin(), ie = end(); it != ie; ++it) {
    for (MCSection::iterator it2 = it->begin(), ie2 = it->end(); it2 != ie2;
      ...
      if (auto *FragWithFixups = dyn_cast<MCDataFragment>(F)) {
        ...
        llvm37_unreachable("Unknown fragment with fixups!");

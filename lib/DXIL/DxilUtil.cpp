...
static void EmitWarningOrErrorOnContext(LLVM37Context &Ctx, Twine Msg, DiagnosticSeverity severity) {
  Ctx.diagnose(DiagnosticInfoDxil(nullptr /*Func*/, nullptr /*DLoc*/,
                                  Msg, severity));
}

void EmitErrorOnContext(LLVM37Context &Ctx, Twine Msg) {
  EmitWarningOrErrorOnContext(Ctx, Msg, DiagnosticSeverity::DS_Error);
}

void EmitWarningOnContext(LLVM37Context &Ctx, Twine Msg) {
  EmitWarningOrErrorOnContext(Ctx, Msg, DiagnosticSeverity::DS_Warning);
}

void EmitNoteOnContext(LLVM37Context &Ctx, Twine Msg) {
...
// Propagates any llvm37.dbg.value instruction for a given vector
...
void TryScatterDebugValueToVectorElements(Value *Val) {
  ...
  while (InsertElementInst *InsertElt = dyn_cast<InsertElementInst>(Val)) {
    ...
    // Offset is basically unused and deprecated in later LLVM37 versions.
...
// Based on the implementation available in LLVM37's trunk:
// http://llvm37.org/doxygen/Constants_8cpp_source.html#l02734

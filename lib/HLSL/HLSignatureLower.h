...
public:
  HLSignatureLower(llvm37::Function *F, HLModule &M, DxilEntrySignature &Sig)
      : Entry(F), HLM(M), EntrySig(Sig) {}
  void Run();

private:
  // Create signatures.
  void ProcessArgument(llvm37::Function *func,
                       DxilFunctionAnnotation *EntryAnnotation,
                       llvm37::Argument &arg, DxilFunctionProps &props,
  ...
  void GenerateClipPlanesForVS(llvm37::Value *outPosition);
  bool HasClipPlanes();
  // Generate DXIL stream output operation.
  void GenerateStreamOutputOperation(llvm37::Value *streamVal, unsigned streamID);
  // Generate DXIL stream output operations.
  void GenerateStreamOutputOperations();
  // Generate DXIL EmitIndices operation.
  void GenerateEmitIndicesOperation(llvm37::Value *indicesOutput);
...
private:
  ...
  std::unordered_map<DxilSignatureElement *, llvm37::Value *> m_sigValueMap;
  ...
  std::unordered_set<llvm37::Value *> m_inoutArgSet;

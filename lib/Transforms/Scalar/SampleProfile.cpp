...
protected:
  ...
  /// \brief LLVM37 context holding the debug data we need.
  LLVM37Context *Ctx;
...
void SampleProfileLoader::buildEdges(Function &F) {
  for (auto &BI : F) {
    ...
    if (!Predecessors[B1].empty())
      llvm37_unreachable("Found a stale predecessors list in a basic block.");
    ...
    if (!Successors[B1].empty())
      llvm37_unreachable("Found a stale successors list in a basic block.");
...
void SampleProfileLoader::propagateWeights(Function &F) {
  ...
  for (auto &BI : F) {
    ...
    if (!AllWeightsZero) {
      ...
      TI->setMetadata(llvm37::LLVM37Context::MD_prof,
...
INITIALIZELLVM37_PASS_BEGIN(SampleProfileLoader, "sample-profile",
...
INITIALIZELLVM37_PASS_DEPENDENCY(AddDiscriminators)
INITIALIZELLVM37_PASS_END(SampleProfileLoader, "sample-profile",
                    "Sample Profile loader", false, false)

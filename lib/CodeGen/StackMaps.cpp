...
StackMaps::StackMaps(AsmPrinter &AP) : AP(AP) {
  if (StackMapVersion != 1)
    llvm37_unreachable("Unsupported stackmap version!");
}
...
StackMaps::parseOperand(MachineInstr::const_mop_iterator MOI,
  ...
  if (MOI->isImm()) {
    switch (MOI->getImm()) {
    default:
      llvm37_unreachable("Unrecognized operand type.");
  ...
  if (MOI->isReg()) {
    ...
    unsigned LLVM37RegNum = TRI->getLLVM37RegNum(DwarfRegNum, false);
    unsigned SubRegIdx = TRI->getSubRegIndex(LLVM37RegNum, MOI->getReg());
    if (SubRegIdx)

...
Value *DxilValueCache::ProcessAndSimplify_PHI(Instruction *I, DominatorTree *DT) {
  ...
  // If we coulnd't deduce it, run the LLVM37 stock simplification to see
...
Value *DxilValueCache::SimplifyAndCacheResult(Instruction *I, DominatorTree *DT) {
  ...
  // The rest of the checks use LLVM37 stock simplifications
...
Value *DxilValueCache::WeakValueMap::GetSentinel(LLVM37Context &Ctx) {
  if (!Sentinel) {

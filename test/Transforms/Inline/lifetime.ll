...
define void @helper_both_markers() {
  %a = alloca i8
  ; Size in llvm37.lifetime.start / llvm37.lifetime.end differs from
  ; allocation size. We should use the former.
  call void @llvm37.lifetime.start(i64 2, i8* %a)
  call void @llvm37.lifetime.end(i64 2, i8* %a)
  ret void
}

define void @test_both_markers() {
; CHECK-LABEL: @test_both_markers(
; CHECK: llvm37.lifetime.start(i64 2
; CHECK-NEXT: llvm37.lifetime.end(i64 2
  call void @helper_both_markers()
; CHECK-NEXT: llvm37.lifetime.start(i64 2
; CHECK-NEXT: llvm37.lifetime.end(i64 2
  call void @helper_both_markers()
; CHECK-NEXT: ret void
  ret void
...
; CHECK-NOT: lifetime
; CHECK: llvm37.lifetime.start(i64 1
; CHECK-NOT: lifetime
; CHECK: llvm37.lifetime.end(i64 1
...
define void @helper_two_casts() {
  ...
  call void @llvm37.lifetime.start(i64 4, i8* %b)
  %c = bitcast i32* %a to i8*
  call void @llvm37.lifetime.end(i64 4, i8* %c)
  ret void
}
...
; CHECK-NOT: lifetime
; CHECK: llvm37.lifetime.start(i64 4
; CHECK-NOT: lifetime
; CHECK: llvm37.lifetime.end(i64 4
...
; CHECK-NOT: lifetime
; CHECK: llvm37.lifetime.start(i64 40,
; CHECK-NOT: lifetime
; CHECK: llvm37.lifetime.end(i64 40,

...
declare %overflow.result @llvm37.uadd.with.overflow.i8(i8, i8) nounwind readnone
declare %overflow.result @llvm37.umul.with.overflow.i8(i8, i8) nounwind readnone
declare %ov.result.32 @llvm37.sadd.with.overflow.i32(i32, i32) nounwind readnone
declare %ov.result.32 @llvm37.uadd.with.overflow.i32(i32, i32) nounwind readnone
declare %ov.result.32 @llvm37.ssub.with.overflow.i32(i32, i32) nounwind readnone
declare %ov.result.32 @llvm37.usub.with.overflow.i32(i32, i32) nounwind readnone
declare %ov.result.32 @llvm37.smul.with.overflow.i32(i32, i32) nounwind readnone
declare %ov.result.32 @llvm37.umul.with.overflow.i32(i32, i32) nounwind readnone
declare double @llvm37.powi.f64(double, i32) nounwind readonly
declare i32 @llvm37.cttz.i32(i32, i1) nounwind readnone
declare i32 @llvm37.ctlz.i32(i32, i1) nounwind readnone
declare i32 @llvm37.ctpop.i32(i32) nounwind readnone
declare i8 @llvm37.ctlz.i8(i8, i1) nounwind readnone
declare double @llvm37.cos.f64(double %Val) nounwind readonly
declare double @llvm37.sin.f64(double %Val) nounwind readonly

define i8 @uaddtest1(i8 %A, i8 %B) {
  %x = call %overflow.result @llvm37.uadd.with.overflow.i8(i8 %A, i8 %B)
...
define i8 @uaddtest2(i8 %A, i8 %B, i1* %overflowPtr) {
  %and.A = and i8 %A, 127
  %and.B = and i8 %B, 127
  %x = call %overflow.result @llvm37.uadd.with.overflow.i8(i8 %and.A, i8 %and.B)
  %y = extractvalue %overflow.result %x, 0
...
define i8 @uaddtest3(i8 %A, i8 %B, i1* %overflowPtr) {
  %or.A = or i8 %A, -128
  %or.B = or i8 %B, -128
  %x = call %overflow.result @llvm37.uadd.with.overflow.i8(i8 %or.A, i8 %or.B)
  %y = extractvalue %overflow.result %x, 0
  %z = extractvalue %overflow.result %x, 1
  store i1 %z, i1* %overflowPtr
  ret i8 %y
...
define i8 @uaddtest4(i8 %A, i1* %overflowPtr) {
  %x = call %overflow.result @llvm37.uadd.with.overflow.i8(i8 undef, i8 %A)
  %y = extractvalue %overflow.result %x, 0
  %z = extractvalue %overflow.result %x, 1
  store i1 %z, i1* %overflowPtr
  ret i8 %y
...
define i8 @uaddtest5(i8 %A, i1* %overflowPtr) {
  %x = call %overflow.result @llvm37.uadd.with.overflow.i8(i8 0, i8 %A)
  %y = extractvalue %overflow.result %x, 0
  %z = extractvalue %overflow.result %x, 1
  store i1 %z, i1* %overflowPtr
  ret i8 %y
...
define i1 @uaddtest6(i8 %A, i8 %B) {
  %x = call %overflow.result @llvm37.uadd.with.overflow.i8(i8 %A, i8 -4)
  %z = extractvalue %overflow.result %x, 1
  ret i1 %z
...
define %ov.result.32 @saddtest_nsw(i8 %a, i8 %b) {
  %A = sext i8 %a to i32
  %B = sext i8 %b to i32
  %x = call %ov.result.32 @llvm37.sadd.with.overflow.i32(i32 %A, i32 %B)
  ret %ov.result.32 %x
...
define %ov.result.32 @uaddtest_nuw(i32 %a, i32 %b) {
  ...
  %x = call %ov.result.32 @llvm37.uadd.with.overflow.i32(i32 %A, i32 %B)
  ret %ov.result.32 %x
...
define %ov.result.32 @ssubtest_nsw(i8 %a, i8 %b) {
  %A = sext i8 %a to i32
  %B = sext i8 %b to i32
  %x = call %ov.result.32 @llvm37.ssub.with.overflow.i32(i32 %A, i32 %B)
  ret %ov.result.32 %x
...
define %ov.result.32 @usubtest_nuw(i32 %a, i32 %b) {
  ...
  %B = and i32 %b, 2147483647
  %x = call %ov.result.32 @llvm37.usub.with.overflow.i32(i32 %A, i32 %B)
  ret %ov.result.32 %x
...
define %ov.result.32 @smultest1_nsw(i32 %a, i32 %b) {
  ...
  %x = call %ov.result.32 @llvm37.smul.with.overflow.i32(i32 %A, i32 %B)
  ret %ov.result.32 %x
...
; CHECK: %x = call %ov.result.32 @llvm37.smul.with.overflow.i32(i32 %A, i32 %B)
...
define %ov.result.32 @umultest_nuw(i32 %a, i32 %b) {
  ...
  %x = call %ov.result.32 @llvm37.umul.with.overflow.i32(i32 %A, i32 %B)
  ret %ov.result.32 %x
...
define i8 @umultest1(i8 %A, i1* %overflowPtr) {
  %x = call %overflow.result @llvm37.umul.with.overflow.i8(i8 0, i8 %A)
  %y = extractvalue %overflow.result %x, 0
  %z = extractvalue %overflow.result %x, 1
  store i1 %z, i1* %overflowPtr
  ret i8 %y
...
define i8 @umultest2(i8 %A, i1* %overflowPtr) {
  %x = call %overflow.result @llvm37.umul.with.overflow.i8(i8 1, i8 %A)
  %y = extractvalue %overflow.result %x, 0
  %z = extractvalue %overflow.result %x, 1
  store i1 %z, i1* %overflowPtr
  ret i8 %y
...
define i32 @umultest3(i32 %n) nounwind {
  %shr = lshr i32 %n, 2
  %mul = call %ov.result.32 @llvm37.umul.with.overflow.i32(i32 %shr, i32 3)
...
define i32 @umultest4(i32 %n) nounwind {
  %shr = lshr i32 %n, 1
  %mul = call %ov.result.32 @llvm37.umul.with.overflow.i32(i32 %shr, i32 4)
  %ov = extractvalue %ov.result.32 %mul, 1
  %res = extractvalue %ov.result.32 %mul, 0
  %ret = select i1 %ov, i32 -1, i32 %res
  ret i32 %ret
...
define %ov.result.32 @umultest5(i32 %x, i32 %y) nounwind {
  ...
  %mul = call %ov.result.32 @llvm37.umul.with.overflow.i32(i32 %or_x, i32 %or_y)
...
entry:
  %A = tail call double @llvm37.powi.f64(double %V, i32 -1) nounwind
  store volatile double %A, double* %P

  %B = tail call double @llvm37.powi.f64(double %V, i32 0) nounwind
  store volatile double %B, double* %P

  %C = tail call double @llvm37.powi.f64(double %V, i32 1) nounwind
...
entry:
  %or = or i32 %a, 8
  %and = and i32 %or, -8
  %count = tail call i32 @llvm37.cttz.i32(i32 %and, i1 true) nounwind readnone
  ret i32 %count
; CHECK-LABEL: @cttz(
; CHECK-NEXT: entry:
...
entry:
  %or = or i8 %a, 32
  %and = and i8 %or, 63
  %count = tail call i8 @llvm37.ctlz.i8(i8 %and, i1 true) nounwind readnone
  ret i8 %count
; CHECK-LABEL: @ctlz(
; CHECK-NEXT: entry:
; CHECK-NEXT: ret i8 2
...
entry:
  %lz = tail call i32 @llvm37.ctlz.i32(i32 %a, i1 false) nounwind readnone
  %lz.cmp = icmp eq i32 %lz, 32
  store volatile i1 %lz.cmp, i1* %c
  %tz = tail call i32 @llvm37.cttz.i32(i32 %a, i1 false) nounwind readnone
  %tz.cmp = icmp ne i32 %tz, 32
  store volatile i1 %tz.cmp, i1* %c
  %pop = tail call i32 @llvm37.ctpop.i32(i32 %b) nounwind readnone
...
define i32 @cttz_simplify1a(i32 %x) nounwind readnone ssp {
  %tmp1 = tail call i32 @llvm37.ctlz.i32(i32 %x, i1 false)
...
define i32 @cttz_simplify1b(i32 %x) nounwind readnone ssp {
  %tmp1 = tail call i32 @llvm37.ctlz.i32(i32 %x, i1 true)
  %shr3 = lshr i32 %tmp1, 5
  ret i32 %shr3
...
define i32 @ctlz_undef(i32 %Value) nounwind {
  %ctlz = call i32 @llvm37.ctlz.i32(i32 0, i1 true)
  ret i32 %ctlz
...
define i32 @cttz_undef(i32 %Value) nounwind {
  %cttz = call i32 @llvm37.cttz.i32(i32 0, i1 true)
  ret i32 %cttz
...
define i32 @ctlz_select(i32 %Value) nounwind {
  ...
  %ctlz = call i32 @llvm37.ctlz.i32(i32 %Value, i1 true)
  %s = select i1 %tobool, i32 %ctlz, i32 32
  ret i32 %s

; CHECK-LABEL: @ctlz_select(
; CHECK-NEXT: call i32 @llvm37.ctlz.i32(i32 %Value, i1 false)
; CHECK-NEXT: ret i32
}

define i32 @cttz_select(i32 %Value) nounwind {
  %tobool = icmp ne i32 %Value, 0
  %cttz = call i32 @llvm37.cttz.i32(i32 %Value, i1 true)
  %s = select i1 %tobool, i32 %cttz, i32 32
  ret i32 %s

; CHECK-LABEL: @cttz_select(
; CHECK-NEXT: call i32 @llvm37.cttz.i32(i32 %Value, i1 false)
; CHECK-NEXT: ret i32
}
...
entry:
  %div = sdiv i32 %v1, 2
  %t = call %ov.result.32 @llvm37.sadd.with.overflow.i32(i32 %div, i32 1)
...
entry:
  ...
  %t = call %ov.result.32 @llvm37.ssub.with.overflow.i32(i32 %div, i32 1)
  %obit = extractvalue %ov.result.32 %t, 1
  ret i1 %obit
}
...
entry:
  ...
  %t = call %ov.result.32 @llvm37.smul.with.overflow.i32(i32 %rem, i32 %rem)
  %obit = extractvalue %ov.result.32 %t, 1
  ret i1 %obit
}
...
define %ov.result.32 @ssubtest_reorder(i8 %a) {
  %A = sext i8 %a to i32
  %x = call %ov.result.32 @llvm37.ssub.with.overflow.i32(i32 0, i32 %A)
  ret %ov.result.32 %x
...
define %ov.result.32 @never_overflows_ssub_test0(i32 %a) {
  %x = call %ov.result.32 @llvm37.ssub.with.overflow.i32(i32 %a, i32 0)
  ret %ov.result.32 %x
...
entry:
  %B = tail call double @llvm37.cos.f64(double 0.0) nounwind
  store volatile double %B, double* %P
...
entry:
  %B = tail call double @llvm37.sin.f64(double 0.0) nounwind
  store volatile double %B, double* %P

...
; Check ffs(x) -> x != 0 ? (i32)llvm37.cttz(x) + 1 : 0.

define i32 @test_simplify13(i32 %x) {
; CHECK-LABEL: @test_simplify13(
  %ret = call i32 @ffs(i32 %x)
; CHECK-NEXT: [[CTTZ:%[a-z0-9]+]] = call i32 @llvm37.cttz.i32(i32 %x, i1 false)
...
; CHECK-LINUX-NEXT: [[CTTZ:%[a-z0-9]+]] = call i32 @llvm37.cttz.i32(i32 %x, i1 false)
...
; CHECK-LINUX-NEXT: [[CTTZ:%[a-z0-9]+]] = call i64 @llvm37.cttz.i64(i64 %x, i1 false)

; Linking these two translation units causes there to be two LLVM37 values in the
...
; little symbol table.  This is causing llvm37-link to die, at no fault of its
; own.

; RUN: llvm37-as < %s > %t.out2.bc
; RUN: echo "%%T1 = type opaque  @GVar = external global %%T1*" | llvm37-as > %t.out1.bc

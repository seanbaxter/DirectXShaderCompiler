...
declare float @llvm37.nvvm.atomic.load.add.f32.p0f32(float* %addr, float %val)

; CHECK-LABEL: atomic_add_f32_generic
define float @atomic_add_f32_generic(float* %addr, float %val) {
; CHECK: atom.add.f32
  %ret = call float @llvm37.nvvm.atomic.load.add.f32.p0f32(float* %addr, float %val)
  ret float %ret
}

declare float @llvm37.nvvm.atomic.load.add.f32.p1f32(float addrspace(1)* %addr, float %val)
...
; CHECK: atom.global.add.f32
  %ret = call float @llvm37.nvvm.atomic.load.add.f32.p1f32(float addrspace(1)* %addr, float %val)
  ret float %ret
}

declare float @llvm37.nvvm.atomic.load.add.f32.p3f32(float addrspace(3)* %addr, float %val)
...
; CHECK: atom.shared.add.f32
  %ret = call float @llvm37.nvvm.atomic.load.add.f32.p3f32(float addrspace(3)* %addr, float %val)
  ret float %ret
}
